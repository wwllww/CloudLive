...
FuturePixelShader GraphicsSystem::CreatePixelShaderFromFileAsync(CTSTR fileName)
{
    ...
    if (!initialized)
    {
        ...
        c.fileName = fn;
		c.System = this;
        c.thread.reset(OSCreateThread(static_cast<XTHREAD>([](LPVOID arg)->DWORD
        {
			...
			c.System->CreatePixelShaderBlob(c.shaderData, strShader.Array(), c.fileName.c_str());
    ...
    return FuturePixelShader(c.readyEvent.get(), c.shaderData, c.fileData, c.fileName,this);
}
