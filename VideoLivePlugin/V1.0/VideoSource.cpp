...
void VideoLiveSource::Render(const Vect2 &pos, const Vect2 &size, Texture *FilterTexture, bool bScaleFull, bool bIsLiveC)
{
	...
	if (texture&&m_bReadyDraw) {
		...
		else
		{
			...
			DrawSprite(texture, (opacity255 << 24) | 0xFFFFFF, /*40, 0, 280, 240*/ pos.x + mediaOffset.x, pos.y + mediaOffset.y, pos.x + mediaSize.x, pos.y + mediaSize.y);
			LoadPixelShader(oldShader);
		}
...
void VideoLiveSource::BeginScene()
{
	...
	GetTextureWH(texture, Width, Heigth);
