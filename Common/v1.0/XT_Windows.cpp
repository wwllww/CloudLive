...
extern INT_PTR BLiveDialogBox(HINSTANCE hInstance, LPCWSTR lpTemplateName, HWND hWndParent, DLGPROC lpDialogFunc, LPARAM dwInitParam = 0);
extern HWND BLiveCreateDialog(HINSTANCE hInstance, LPCWSTR lpTemplateName, HWND hWndParent, DLGPROC lpDialogFunc, LPARAM dwInitParam = 0);
...
// 
//     //current checks:
//     //TeamSpeak 3 Overlay (hooks CreateDXGIFactory1 in such a way that it fails when called by BLive)
//     //Webroot Secureanywhere (hooks GDI calls and prevents BLive from screen capturing among other issues)
// 
...
//     if (ts3_hook_present)
//     {
//         errors << TEXT("TeamSpeak 3 overlay has loaded into BLive and will cause crashing and instability. Please set \"Disable Loading\" for BLive.EXE in your TeamSpeak 3 overlay settings or visit http://bit.ly/BLiveTS3 for help."); 
//         ret = TRUE;
//     }
// 
...
//                             errors << TEXT("\r\n\r\n");
// 
//                         errors << TEXT("Webroot Secureanywhere appears to be active. This product is incompatible with BLive as the security features block BLive from accessing Windows GDI functions. Please add BLive.exe to the Secureanywhere exceptions list and restart BLive - see http://bit.ly/BLiveWR if you need help."); 
//                         ret = TRUE;
//                     }
//                 }
//             }
...
//     StringList  moduleList;
//     
//     //Modules that will likely cause BLive to crash because they hooked it.
...
//             CrashError (TEXT("ASUS GamerOSD has been detected. This program installs user mode driver hooks which will corrupt the Direct3D state and crash BLive. Please uninstall ASUS GamerOSD if you wish to use BLive."));
//         }
//     }
// 
//     return 0;
...
//     }
//     //FIXME: add a version check for bigfoot networks LSP (bfllr.dll), crashes BLive on old versions.
// }

OSFileChangeData * STDCALL OSMonitorFileStart(String path, bool suppressLogging)
{
    HANDLE hDirectory;
	OSFileChangeData *data = (OSFileChangeData *)Allocate_Bak(sizeof(*data));
