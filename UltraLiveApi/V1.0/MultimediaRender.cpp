...
void CMultimediaRender::VideoRender(unsigned char* Buffer, int width, int height, int pixformat)
{
	...
	if (pixformat == ColorType_RGB24)
	{
		if (texture == NULL)
		{
			...
			texture = CreateTexture(m_size.x, m_size.y, GS_BGR, textureData, FALSE, FALSE);
			Free(textureData);
		}
	...
	else if ( pixformat == ColorType_RGBA32REVERSE )
	{
		if (texture == NULL)
		{
			texture = CreateTexture(m_size.x, m_size.y, GS_BGRA, nullptr, FALSE, FALSE);
		}
	...
	else if (pixformat == ColorType_I420 || pixformat == ColorType_YV12)
	{
		if (texture == NULL)
		{
			texture = CreateTexture(m_size.x, m_size.y, GS_RGBA, nullptr, FALSE, FALSE);
		}
	...
	else if (pixformat == ColorType_YUY2)
	{
		if (texture == NULL)
		{
			...
			msetd(textureData, 0xFF0000FF, m_size.x * m_size.y * 4);
			texture = CreateTexture(m_size.x, m_size.y, GS_RGB, textureData, FALSE, FALSE);
			Free(textureData);
		}
...
void CMultimediaRender::UpLoad()
{
	if (Pixformat == ColorType_RGB24)
	{
		SetImage(texture,m_pDes, GS_IMAGEFORMAT_BGR, m_size.x * 3);
	}
	else if (Pixformat == ColorType_RGBA32REVERSE)
	{
		SetImage(texture,m_pDes, GS_IMAGEFORMAT_BGRA, m_size.x * 4);
	}
	else if (Pixformat == ColorType_RGB)
	{
		SetImage(texture,m_pDes, GS_IMAGEFORMAT_BGRX, m_size.x * 4);
	}
