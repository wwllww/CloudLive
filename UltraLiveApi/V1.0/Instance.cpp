...
CInstanceProcess::~CInstanceProcess()
{
	...
	if (LittleRenderTarget)
		delete LittleRenderTarget;
				...
				*StreamID1 = (uint64_t)BaseStream;
				...
				if (bLittlePre)
				{
					if (!bNoPreView)
					{
						...
						MultiRender->SetVideoRender(Vect2(0, 0), Vect2(Rect.right, Rect.bottom));
					}
...
{
	SetRenderTarget(SwapRender);

	Ortho(0.0f, renderCtrlSize.x, renderCtrlSize.y, 0.0f, -100.0f, 100.0f);
	SetViewport(0.0f, 0.0f, renderCtrlSize.x, renderCtrlSize.y);


	ClearRenderTarget(0xFF171718);

	DrawSprite(Prev, 0xFFFFFFFF,
	...
	for (int i = 0; i < m_VideoList.Num(); ++i)
	{
		...
		if (m_VideoList[i].bRender && strcmp(OneStruct.VideoStream->GainClassName(), "AgentSource") == 0 && !OneStruct.VideoStream->GetGlobalSource())
		{
			...
			SetCropping(crop.x, crop.y, crop.w, crop.z);
			DrawSprite(OneStruct.VideoStream->GetTexture(), 0xFFFFFFFF,
				pos.x, pos.y,
	...
	{
		EnableBlending(TRUE);
		BlendFunction(GS_BLEND_SRCALPHA, GS_BLEND_INVSRCALPHA, 1.0f);
		DWORD Width, Height;
		GetTextureWH(SDITexture, Width, Height);
		DrawSprite(SDITexture, 0xFFFFFFFF, renderFrameOffset.x + renderFrameSize.x - Width, renderFrameOffset.y, renderFrameOffset.x + renderFrameSize.x , renderFrameOffset.y + Height);
...
{
	EnableBlending(TRUE);
	BlendFunction(GS_BLEND_SRCALPHA, GS_BLEND_INVSRCALPHA, 1.0f); //特效中切入直播混合不生效又加一次
		...
		if (OneVideo.VideoStream && OneVideo.bRender)
		{
			...
			for (int j = 0; j < m_FilterTransForm.Num(); ++j)
			{
				if (m_FilterTransForm[j].IVideo == OneVideo.VideoStream.get() || Agent == m_FilterTransForm[j].IVideo)
				{
					...
					SetRenderTarget(m_FilterTransForm[j].RenderTarget.get());
					ClearRenderTarget(0xFF000000);

					Ortho(0.0f, Size.x, Size.y, 0.0f, -100.0f, 100.0f);
			...
			if (LastTexture)
			{
				SetRenderTarget(PreTexture);
				if (!bLittlePre)
				{
					Ortho(0.0f, baseCX, baseCY, 0.0f, -100.0f, 100.0f);
					SetViewport(0, 0, baseCX, baseCY);
				}
				else
				{
			...
			for (int j = 0; j < m_Filter.Num(); ++j)
			{
				if (m_Filter[j].IVideo == OneVideo.VideoStream.get() || Agent == m_Filter[j].IVideo)
				{
					...
					if (Size != Vect2(m_Filter[j].Width, m_Filter[j].Height))
					{
						...
						m_Filter[j].RenderTarget.reset(CreateRenderTarget(Size.x, Size.y, GS_BGRA, FALSE));
					}
					SetRenderTarget(m_Filter[j].RenderTarget.get());
					ClearRenderTarget(0xFF000000);
...
void CInstanceProcess::CreateLittleRenderTarget()
{
	...
	LittleRenderTarget = CreateRenderTarget(Rect.right, Rect.bottom, GS_RGBA, FALSE);
}

void CInstanceProcess::DrawLittlePreview(Shader *MainVertShader, Shader* MainPixShader, Texture *SDITexture)
{
	SetRenderTarget(SwapRender);

	if (MultiRender)
	{
		Ortho(0.0f, MultiRender->m_size.x, MultiRender->m_size.y, 0.0f, -100.0f, 100.0f);
		SetViewport(0.0f, 0.0f, MultiRender->m_size.x, MultiRender->m_size.y);
		ClearRenderTarget(0xFF000000);

		if (MultiRender->texture)
		{
			LoadVertexShader(MainVertShader);
			if (MultiRender->Pixformat == ColorType_I420 || MultiRender->Pixformat == ColorType_YV12
				|| MultiRender->Pixformat == ColorType_YUY2 || MultiRender->Pixformat == ColorType_UYVY)
			{
				MultiRender->ResetShaderYV12();

				LoadPixelShader(MultiRender->colorConvertShader);
			}
			else
				LoadPixelShader(MainPixShader);

			MultiRender->m_previewlock.Lock();
			MultiRender->UpLoad();
			DrawSprite(MultiRender->texture, 0xFFFFFFFF, 0, 0, MultiRender->m_size.x, MultiRender->m_size.y);

			if (MultiRender->HaveSDIOut())
			{
				LoadPixelShader(MainPixShader);
				DWORD Width, Height;
				GetTextureWH(SDITexture, Width, Height);
				EnableBlending(TRUE);
				BlendFunction(GS_BLEND_SRCALPHA, GS_BLEND_INVSRCALPHA, 1.0f);
				DrawSprite(SDITexture, 0xFFFFFFFF, MultiRender->m_size.x - Width, 0, MultiRender->m_size.x, Height);
				EnableBlending(FALSE);
			}
			MultiRender->m_previewlock.UnLock();
		}
	}
	Present(SwapRender);
}
	...
	if (bFind)
	{
		...
		if (Stream)
		{
			...
			if (!bFindFilter)
			{
				...
				SFilter.RenderTarget = shared_ptr<Texture>(CreateRenderTarget(SFilter.Width, SFilter.Height, GS_BGRA, FALSE));
...
void CInstanceProcess::FindDifAudioWithInstance(CInstanceProcess *Other)
{
	m_DifAudioList.clear();
	...
	for (UINT i = 0; i < m_AudioList.Num(); ++i)
	{
		...
		EnterCriticalSection(&Other->AudioSection);

		for (UINT j = 0; j < Other->m_AudioList.Num(); ++j)
		{
			AudioStruct &OneOtherAudio = m_AudioList[j];

			if (OneAudio.AudioStream && (OneAudio.AudioStream.get() == OneOtherAudio.AudioStream.get()))
			{
				bFind = true;
				break;
			}
		...
		LeaveCriticalSection(&Other->AudioSection);

		if (!bFind)
		{
			m_DifAudioList.push_back(OneAudio.AudioStream.get());
		}
	}
...
void CInstanceProcess::RemveDifAudio()
{
	if (m_DifAudioList.size())
	{
		for (IBaseAudio *Audio : m_DifAudioList)
		{
			EnterCriticalSection(&AudioSection);
