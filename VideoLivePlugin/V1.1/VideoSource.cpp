	...
	if (frame_type == 2) //MPFrameInfo类型
	{
		...
		if (pMPFrameInfo->media_type == 1)//视频
		{
			...
			//	Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,iLastVideoPts = %d,This =0x%p"), __LINE__, __FUNCTION__, pMPFrameInfo->pts,This_);
				...
				{
					This_->m_iYUVCount = 0;
					Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_hYUVBufferEvent 缓冲成功This =0x%p"), __LINE__, __FUNCTION__,This_);
		...
		else if (pMPFrameInfo->media_type == 2)//音频
		{
			...
			if (This_->m_StatusStream == PCM_STREAM) {
				...
				{
					This_->m_iPCMCount = 0;
					Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_hPCMBufferEvent 缓冲成功This =0x%p"), __LINE__, __FUNCTION__,This_);
...
void VideoLiveSource::SetParamCallBackFunc(int nMediatype, void* ctx)
{
	...
	if (This_->m_StatusStream != STOP_STREAM) {
		...
		Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_hH264AACBufferEvent 缓冲成功,This =0x%p"), __LINE__, __FUNCTION__, This_);
	}
}
...
void VideoLiveSource::UpdateSettings(Value &JsonParam)
{
	...
	Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,响应用户输入消息!m_StatusStream = %d.This =0x%p"), __LINE__, __FUNCTION__, m_StatusStream, this);
	if (H264_AAC_STREAM == m_StatusStream) {         //当前正处在缓冲码流
		m_StatusStream = STOP_STREAM;
		SetEvent(m_hH264AACBufferEvent);
	}
		...
		if (m_bAllowClose)
		{
		Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,无法响应用户输入消息,返回!m_bAllowClose = %d.m_StatusStream = %d.This =0x%p"), __LINE__, __FUNCTION__, m_bAllowClose,m_StatusStream, this);
		return;
		}
		}*/
		m_StatusStream = STOP_STREAM;
...
void VideoLiveSource::Synchronization()
{
	...
	while (true){
		...
		if (m_bAllowClose)
		{
			...
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,进入 m_hPauseThreadEvent 锁等待Thread = %d,This =0x%p"),
				__LINE__, __FUNCTION__, GetCurrentThreadId(), this);
			WaitForSingleObject(m_hPauseThreadEvent, INFINITE);
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,走出 m_hPauseThreadEvent 锁等待Thread = %d,This =0x%p"),
				__LINE__, __FUNCTION__, GetCurrentThreadId(), this);
	    ...
	    if (m_FirstVideoFrameTime > 0 && m_CurrentVideoFrameTime > 0)
		{
			...
			if (iLogIndex > 550 && iLogIndex < 560)
				Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s 计算视频帧时间增量.m_CurrentVideoFrameTime = %lld,m_FirstVideoFrameTime = %lld,m_DiffVideoFrameTime = %lld.Thread = %d,This =0x%p"),
		...
		if (m_FirstQpcTime > 0)
		{
			...
			if (iLogIndex > 550 && iLogIndex < 560)
				Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s 计算视渲染时间增量.m_CurrentQpcTime = %lld,m_FirstQpcTime = %lld.m_DiffQpcTime = %lld.Thread = %d,This =0x%p"),
		...
		if (m_DiffQpcTime / 1000000 - m_DiffVideoFrameTime > 10 && !m_bResetDiffMode)  //不是时戳跳变情况下的处理
		{
			...
			if (iLogIndex > 550 && iLogIndex < 560)
				Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s m_DiffQpcTime / 1000000 = %lld.m_DiffVideoFrameTime = %lld,调整渲染睡眠时间,frameTimeNS = %d.Thread = %d,This =0x%p"),
		...
		{
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s.当前帧发生了跳变，将所有计时器重新初始化!Thread = %d,This =0x%p"), __LINE__, __FUNCTION__,GetCurrentThreadId(), this);
...
//重推处理，获取前两帧
		if (m_bisReBuffer) //刚刚重新缓冲过
		{
			...
			{
				Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_YUVBuffer获取第一帧失败,Thread = %d,This =0x%p"), __LINE__, __FUNCTION__, GetCurrentThreadId(), this);
				m_StatusStream = STOP_STREAM;
				frameTimeNS = 40;
				SetEvent(m_hUpdataEvent);
				continue;
			...
			if (!Last_inf_Video)
			{
				Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,Last_inf_Video 指针为空了!  m_YUVBuffer.size = %dThis =0x%p"), __LINE__, __FUNCTION__, m_YUVBuffer.unsafe_size(), this);
				continue;
			}
			...
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,第一帧开始时间戳: %lld,This =0x%p"), __LINE__, __FUNCTION__, m_FirstVideoFrameTime, this);
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,第一帧开始渲染时间: %lld,This =0x%p"), __LINE__, __FUNCTION__, m_FirstQpcTime, this);
			
			if (m_iYUVBufferTotalTime <1000 || m_iYUVBufferTotalTime > 2000)
			{
				Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,yuv缓冲时长m_iYUVBufferTotalTime = %d，不符合要求进行调整为1秒,Thread = %d,This =0x%p"), __LINE__, __FUNCTION__, m_iYUVBufferTotalTime, GetCurrentThreadId(), this);
				m_iYUVBufferTotalTime = 1000; //只缓冲1秒数据
				
			}
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,yuv缓冲时长m_iYUVBufferTotalTime = %d，Thread = %d,This =0x%p"), __LINE__, __FUNCTION__, m_iYUVBufferTotalTime, GetCurrentThreadId(), this);
...
//上一帧时戳
		...
		{
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_YUVBuffer缓冲区空了重新打开,Thread = %d,This =0x%p"), __LINE__, __FUNCTION__, GetCurrentThreadId(), this);
		...
		{
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,inf_Video 指针为空了!  m_YUVBuffer.size = %dThis =0x%p"), __LINE__, __FUNCTION__, m_YUVBuffer.unsafe_size(), this);
			frameTimeNS = 40;
			continue;
		}
		...
		if (iLogIndex > 550 && iLogIndex < 560)
		{
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,渲染视频iLastVideoPts = %d, mp_get_frame  m_YUVBuffer.size = %d,This =0x%p"), __LINE__, __FUNCTION__, iLastVideoPts, m_YUVBuffer.unsafe_size(),this);
		}
		...
		{
			//删除当前音频并且重新获取包括残留音频帧
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,时间戳发生变化查看是否重推,iLastVideoPts = %d,iCurrentVideoPts = %d,Thread = %d,This =0x%p"), __LINE__, __FUNCTION__, iLastVideoPts, iCurrentVideoPts, GetCurrentThreadId(), this);
		...
		{
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,frameTimeNS = %d,diff = %lld,时戳跳变太大，重新进行缓冲.Thread = %d,This =0x%p"), __LINE__, __FUNCTION__, frameTimeNS, CT - LT, GetCurrentThreadId(), this);
			if (Last_inf_Audio)
			{
				Last_inf_Audio->Release();
				Last_inf_Audio = NULL;
		...
		if (frameTimeNS > (1000/m_pMPMediaInfo.v_frame_rate)*5 )  //时戳跳变大于5倍帧率，需要进行探测
		{
			...
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,需要探测音频。frameTimeNS = %d,iCurrentVideoPts = %d,iLastVideoPts = %d,diff = %lld,m_fVideoWarnning = %d,Thread = %d,This =0x%p"), __LINE__, __FUNCTION__, frameTimeNS, iCurrentVideoPts, iLastVideoPts, CT - LT, m_fVideoWarnning, GetCurrentThreadId(), this);
		}
//--------------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------
		if (Last_inf_Audio&&Last_inf_Audio->lpData && (Last_inf_Audio->timestamp >= iLastVideoPts) &&( Last_inf_Audio->timestamp <= iCurrentVideoPts)) {
			...
			if (m_pDemandMediaAudio)
			{
				...
				if (iLogIndex > 550 && iLogIndex < 560)
					Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,渲染音频audio_pts = %d, mp_get_frame  m_PCMuffer.size = %d,This =0x%p"), __LINE__, __FUNCTION__, audio_pts, m_PCMuffer.unsafe_size(), this);
			}
			Last_inf_Audio->Release();
			Last_inf_Audio = NULL;
		}
		else if (Last_inf_Audio && (Last_inf_Audio->timestamp > iCurrentVideoPts))
		{
			...
			if(iLogIndex > 550 && iLogIndex < 560)
			{
				...
				{
					Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,流严重不同步Last_inf_Audio->timestamp = %d,iLastVideoPts = %d,Thread = %d,This =0x%p"), __LINE__, __FUNCTION__, Last_inf_Audio->timestamp, iLastVideoPts, GetCurrentThreadId(), this);
				}
			}
		...
		if (!m_pMPMediaInfo.has_video) {
			...
			while (true) {
				...
				if (!m_PCMuffer.try_pop(inf_Audio)) {         //重新get
					...
					if(iLogIndex > 550 && iLogIndex < 560)
						Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,音频空了，重新mp_get_frame  m_PCMuffer.unsafe_size() = %dThis =0x%p"), __LINE__, __FUNCTION__, audio_pts, m_PCMuffer.unsafe_size(), this);
					//重新get 音频
				...
				if (audio_pts < iLastVideoPts) {
					mp_get_frame(HMediaProcess, 2, m_fAudioWarnning);
					Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,时戳过小删除audio_pts = %d,iLastVideoPts = %d, mp_get_frame  m_PCMuffer.unsafe_size() = %d,This =0x%p"), __LINE__, __FUNCTION__, audio_pts, iLastVideoPts, m_PCMuffer.unsafe_size(), this);
					...
					if (bisAudioNeedTry)  //需要探测
					{
						if (audioStartTryTimestamp > 0)  //正常情况
						{
							...
							if (frameTimeNS > audioStartTryTimestamp)
							{
								...
								Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,探测音频播放时长。audiolen = %d.audio Time = %d,frameTimeNS = %d,iCurrentVideoPts = %d,iLastVideoPts = %d,diff = %lld,Thread = %d,This =0x%p"), __LINE__, String(__FUNCTION__).Array(), nLen, audioStartTryTimestamp, frameTimeNS, iCurrentVideoPts, iLastVideoPts, CT - LT, GetCurrentThreadId(), this);
							}
							else
							{
								frameTimeNS = audioStartTryTimestamp;
								Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,大于视频时长，探测音频播放时长。audiolen = %d.audio Time = %d,frameTimeNS = %d,iCurrentVideoPts = %d,iLastVideoPts = %d,diff = %lld,Thread = %d,This =0x%p"), __LINE__, String(__FUNCTION__).Array(), nLen, audioStartTryTimestamp, frameTimeNS, iCurrentVideoPts, iLastVideoPts, CT - LT, GetCurrentThreadId(), this);
							}
						}
					}
					break;
...
void VideoLiveSource::Monitor()
{
	...
	while (TRUE){
		...
		if (WAIT_OBJECT_0 == ret) {
			...
			{
				m_bAllowClose = true;
				Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,进入 m_hAllowCloseEvent 锁等待Thread = %d,This =0x%p"),
					__LINE__, __FUNCTION__, GetCurrentThreadId(), this);
			...
			Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,缓冲时间:%d.!"), __LINE__, __FUNCTION__, m_nBufferTime);
			...
			if (WAIT_OBJECT_0 == ret) {
				//...成功
				Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_hH264AACBufferEvent 返回This =0x%p"), __LINE__, __FUNCTION__,this);
				HASSTOP
				ResetEvent(m_hH264AACBufferEvent);
				if (STOP_STREAM == m_StatusStream) {     //外部置重连
					SetEvent(m_hUpdataEvent);//重连
					Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_hH264AACBufferEvent 外部设置,This =0x%p"), __LINE__, __FUNCTION__,this);
					continue;
				}
			}
			else if (WAIT_TIMEOUT == ret){
				HASSTOP
				Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_hH264AACBufferEvent 超时,This =0x%p"), __LINE__, __FUNCTION__,this);
				ResetEvent(m_hH264AACBufferEvent);
				m_StatusStream = STOP_STREAM;
				SetEvent(m_hUpdataEvent);//重连
				continue;
			...
			if (m_pMPMediaInfo.has_video) {
				...
				if (WAIT_OBJECT_0 == ret) {
					Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_hYUVBufferEvent 缓冲成功This =0x%p"), __LINE__, __FUNCTION__,this);
					HASSTOP
					ResetEvent(m_hYUVBufferEvent);
					if (STOP_STREAM == m_StatusStream) {     //外部置重连
						Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_hYUVBufferEvent 外部重置This =0x%p"), __LINE__, __FUNCTION__,this);
						SetEvent(m_hUpdataEvent);//重连
						continue;
					}
						...
						HASSTOP
						Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_hYUVBufferEvent 超时This =0x%p"), __LINE__, __FUNCTION__,this);
						ResetEvent(m_hYUVBufferEvent);
						m_StatusStream = STOP_STREAM;
						SetEvent(m_hUpdataEvent);//重连
						continue;
			...
			if (m_pMPMediaInfo.has_audio) {
				...
				if (WAIT_OBJECT_0 == ret) {
					Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_hPCMBufferEvent 缓冲成功This =0x%p"), __LINE__, __FUNCTION__,this);
					HASSTOP
					ResetEvent(m_hPCMBufferEvent);
					if (STOP_STREAM == m_StatusStream) {     //外部置重连
						Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_hPCMBufferEvent 外部重连This =0x%p"), __LINE__, __FUNCTION__,this);
						SetEvent(m_hUpdataEvent);//重连
						continue;
					}
				} else if (WAIT_TIMEOUT == ret){
						Log::writeMessage(LOG_RTSPSERV, 1, ("LINE : %d, FUNC : %s ,m_hPCMBufferEvent 超时"), __LINE__, __FUNCTION__);
						HASSTOP
						ResetEvent(m_hPCMBufferEvent);
						m_StatusStream = STOP_STREAM;
						SetEvent(m_hUpdataEvent);//重连
