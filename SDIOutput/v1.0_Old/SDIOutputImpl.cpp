...
{
	if (pixformat == ColorFormat_RGB32)
...
void YUV420ToUYVY(unsigned char* pSrcImg, unsigned char* pDstImg, int width, int height)
{
	unsigned char* output = pDstImg;
	unsigned char* input = pSrcImg;
	unsigned char* input2 = input + (width*height);
	unsigned char* input3 = input2 + (width*height / 4);
...
// 	fp5 = fopen("5.pcm", "wb");
	...
	Log::writeMessage(LOG_SDI, 1, "SDI_StartOut Begin! nBufferTime = %d",nBufferTime);
	...
	// 				int a = 0;
		...
		if (pCurDLOutput->EnableVideoOutput(pDLDisplayMode->GetDisplayMode(), /*bmdVideoOutputFlagDefault*/bmdVideoOutputVANC) != S_OK)
		{
		...
		HRESULT hr;
		IDeckLinkMutableVideoFrame* pCurDLVideoFrame = NULL;
		if (nColorFormat == ColorFormat_RGB32 || nColorFormat == ColorFormat_RGBA32REVERSE || nColorFormat == ColorFormat_RGB24)
		{
			hr = pCurDLOutput->CreateVideoFrame(channelInfo->uiFrameWidth, channelInfo->uiFrameHeigh, channelInfo->uiFrameWidth * 4, bmdFormat8BitBGRA, bmdFrameFlagFlipVertical, &pCurDLVideoFrame);
		}
		else if (nColorFormat == ColorFormat_UYVY)
		{
			hr = pCurDLOutput->CreateVideoFrame(channelInfo->uiFrameWidth, channelInfo->uiFrameHeigh, channelInfo->uiFrameWidth * 2, bmdFormat8BitYUV, bmdFrameFlagDefault, &pCurDLVideoFrame);
		}
		else
		{
			hr = pCurDLOutput->CreateVideoFrame(channelInfo->uiFrameWidth, channelInfo->uiFrameHeigh, channelInfo->uiFrameWidth * 2, bmdFormat8BitYUV, bmdFrameFlagFlipVertical, &pCurDLVideoFrame);
		}
		if (hr == S_OK)
			channelInfo->pDLVideoFrame = pCurDLVideoFrame;
		else
		{
			Log::writeError(LOG_SDI, 1, "SDI_StartOut CreateVideoFrame Error!");
			SAFE_RELEASE(pDLIterator);
			SAFE_RELEASE(pCurDLOutput);
			SAFE_RELEASE(pCurDL);
			LeaveCriticalSection(&lock);
			return errCreateVideoFrame;
		}

		channelInfo->colorFormat = nColorFormat;
		channelInfo->displayMode = mode;
		channelInfo->uiTotalFrames = 0;
...
// 		}
		ResetFrame(nDeviceID);
		SetPreroll(nDeviceID, nBufferTime);
...
// 								fwrite(pos->convertBufferShort, nSampleCount * nChannel * sizeof(short), 1, fp5);
		...
		else
		{
			...
			void*	pFrame;
			pChannelInfo->pDLVideoFrame->GetBytes((void**)&pFrame);
			...
			pChannelInfo->pDLOutput->ScheduleVideoFrame(pChannelInfo->pDLVideoFrame, pChannelInfo->uiTotalFrames * pChannelInfo->frameDuration, pChannelInfo->frameDuration, pChannelInfo->frameTimescale);
			pChannelInfo->uiTotalFrames++;
	...
	if (listChannelInfo.size())
	{
		...
		for (; pos != end; ++pos)
		{
			if ((*pos)->nDeviceID == nDeviceID)
			{
				...
				SAFE_RELEASE((*pos)->pDLVideoFrame);
				SAFE_RELEASE((*pos)->pDLOutput);
// 				SAFE_RELEASE((*pos)->pOutputCallback);
				SAFE_RELEASE((*pos)->pDL);
...
void SDIOutput::ResetFrame(int nDeviceID)
{
	if (listChannelInfo.size())
	{
		std::list<ChannelInfo*>::iterator pos = listChannelInfo.begin();
		...
		for (; pos != end; ++pos)
		{
			...
			{
				void*	pFrame;
				(*pos)->pDLVideoFrame->GetBytes((void**)&pFrame);
				memset(pFrame, 0x00, (*pos)->pDLVideoFrame->GetRowBytes() * (*pos)->uiFrameHeigh);
				break;
			}
		}
	}
...
void SDIOutput::SetPreroll(int nDeviceID, int bufferTime)
{
	...
	if (listChannelInfo.size())
	{
		...
		for (; pos != end; ++pos)
		{
			int nBufferFrame = 1;
			if (bufferTime > 0)
				nBufferFrame = (bufferTime * (*pos)->uiFPS / 1000) * 2;

			Log::writeMessage(LOG_SDI, 1, "%s nBufferFrame = %d", __FUNCTION__, nBufferFrame);

			if ((*pos)->nDeviceID == nDeviceID)
			{
				for (unsigned __int32 i = 0; i < nBufferFrame; i++)
				{
					if ((*pos)->pDLOutput->ScheduleVideoFrame((*pos)->pDLVideoFrame, ((*pos)->uiTotalFrames * (*pos)->frameDuration), (*pos)->frameDuration, (*pos)->frameTimescale) != S_OK)
						return;
...
bool SDIOutput::Restart(bool bAudio, ChannelInfo* pChannelInfo, int nChannel, int nSample, bool bParaChange, SDIOUT_COLORFORMAT colorFormat, int nWidth, int nHeight)
{
	...
	else
	{
		...
		{
			if (pChannelInfo->pConvertBuf)
			{
				delete[]pChannelInfo->pConvertBuf;
				pChannelInfo->pConvertBuf = NULL;
			}

			SAFE_RELEASE(pChannelInfo->pDLVideoFrame);

			HRESULT hr;
			if (colorFormat == ColorFormat_RGB32 || colorFormat == ColorFormat_RGBA32REVERSE || colorFormat == ColorFormat_RGB24)
				hr = pChannelInfo->pDLOutput->CreateVideoFrame(pChannelInfo->uiFrameWidth, pChannelInfo->uiFrameHeigh, pChannelInfo->uiFrameWidth * 4, bmdFormat8BitBGRA, bmdFrameFlagFlipVertical, &pChannelInfo->pDLVideoFrame);
			else if (colorFormat == ColorFormat_UYVY)
				hr = pChannelInfo->pDLOutput->CreateVideoFrame(pChannelInfo->uiFrameWidth, pChannelInfo->uiFrameHeigh, pChannelInfo->uiFrameWidth * 2, bmdFormat8BitYUV, bmdFrameFlagDefault, &pChannelInfo->pDLVideoFrame);
			else
				hr = pChannelInfo->pDLOutput->CreateVideoFrame(pChannelInfo->uiFrameWidth, pChannelInfo->uiFrameHeigh, pChannelInfo->uiFrameWidth * 2, bmdFormat8BitYUV, bmdFrameFlagFlipVertical, &pChannelInfo->pDLVideoFrame);
