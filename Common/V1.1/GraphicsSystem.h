...
template <Shader *CreateShader(ShaderBlob const&, CTSTR lpShader, CTSTR lpFileName, GraphicsSystem *System )>
class  FutureShader
{
	...
	FutureShader(HANDLE event_, ShaderBlob const &data, std::wstring const &fileData, std::wstring fileName, GraphicsSystem *_System) : shaderData(&data), fileData(&fileData), fileName(fileName), shaderReadyEvent(event_), System(_System) {}
public:
    FutureShader() {}
    FutureShader(FutureShader &&other)
		...
		isReady(other.isReady), System(other.System)
    {
    ...
    inline Shader *Shader()
    {
        ...
        if (!isReady)
        {
            ...
            shader.reset(CreateShader(*shaderData, fileData->c_str(), fileName.c_str(),System));
            isReady = true;
        }
        return shader.get();
    }
...
inline Shader* CreateVertexShaderFromBlob(ShaderBlob const& blob, CTSTR lpShader, CTSTR lpFileName,GraphicsSystem *System);
inline Shader* CreatePixelShaderFromBlob(ShaderBlob const& blob, CTSTR lpShader, CTSTR lpFileName, GraphicsSystem *System);
...
public:
	...
	virtual void  initD3D(UINT adapterID) = 0;
...
inline Shader* CreateVertexShaderFromBlob(ShaderBlob const& blob, CTSTR lpShader, CTSTR lpFileName, GraphicsSystem *System) { return System->CreateVertexShaderFromBlob(blob, lpShader, lpFileName); }
inline Shader* CreatePixelShaderFromBlob(ShaderBlob const& blob, CTSTR lpShader, CTSTR lpFileName, GraphicsSystem *System)  { return System->CreatePixelShaderFromBlob(blob, lpShader, lpFileName); }
