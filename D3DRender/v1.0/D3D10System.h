...
class D3D10VertexBuffer 
{
	...
	static D3D10VertexBuffer* CreateVertexBuffer(VBData *vbData, BOOL bStatic);
...
class D3D10Texture : public Texture
{
	...
	static Texture* CreateTextureFromFile(CTSTR lpFile, BOOL bBuildMipMaps, UINT &Width, UINT& Height);
	...
	static Texture* CreateTextureRead(unsigned int width, unsigned int height);
public:
    ~D3D10Texture();
};
...
class D3D10VertexShader : public D3D10Shader
{
	...
	static Shader* CreateVertexShaderFromFile(CTSTR lpFileName);
...
class D3D10PixelShader : public D3D10Shader
{
	...
	static Shader* CreatePixelShaderFromFile(CTSTR lpFileName);
...
class D3D10System : public GraphicsSystem
{
	...
	D3D10VertexBuffer            *spriteVertexBuffer, *boxVertexBuffer;
