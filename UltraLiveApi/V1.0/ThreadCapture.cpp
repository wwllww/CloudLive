	...
	while (bRunning)
	{
		while (WaitForSingleObject(hVideoEvent, INFINITE) == WAIT_OBJECT_0)
		{
			...
			for (int i = 0; i < m_InstanceList.GetSize(); ++i)
			{
				...
				SetViewport(0, 0, baseCX, baseCY);// 一定要在PreProcess之后

				if (Process->IsLiveInstance)
				{
					if (!bTransUpDown && !bTransDiffuse)
					{
						SetRenderTarget(mainRenderTextures[swapIndex]);
					...
					if (bTransDisSolving || bTransUpDown || bTransDiffuse || bRadius)
					{
						...
						SetRenderTarget(transNewTexture);
						...
						if (bTransDisSolving)
						{
							...
							LoadPixelShader(transitionPixel);

							transitionPixel->SetFloat(hTransitionTime, TransEscapeTime);
							LoadTexture(transNewTexture, 1U);

							DrawSpriteEx(mainRenderTextures[swapIndex], 0xFFFFFFFF,
						...
						{
							LoadSamplerState(ss, 0);
							DrawSpriteEx(transNewTexture, 0xFFFFFFFF,
						...
						{
							Shader *oldPixelShader = GetCurrentPixelShader();
							LoadPixelShader(circleTransitionPixel);
				...
				else
				{
					...
					{
						SetRenderTarget(Process->LittleRenderTarget);
						Ortho(0.0f, Process->renderFrameCtrlSize.x, Process->renderFrameCtrlSize.y, 0.0f, -100.0f, 100.0f);
						SetViewport(0, 0, Process->renderFrameCtrlSize.x, Process->renderFrameCtrlSize.y);

						Process->DrawRender(Process->LittleRenderTarget, mainVertexShader, mainPixelShader);
						RenderTexture = Process->LittleRenderTarget;
					}
					else
					{
						//profileIn("MainVideoLoop Render")
						SetRenderTarget(PreRenderTexture);
...
// 							}
			...
			try{
				...
				if (bStartLive)
				{
					...
					SetRenderTarget(yuvRenderTextures);
					...
					Ortho(0.0f, outputCX, outputCY, 0.0f, -100.0f, 100.0f);
					SetViewport(0.0f, 0.0f, outputCX, outputCY);

					if (!bLiveIsHaveFieldSignal)
					{
						if (bTransDisSolving || bTransUpDown || bTransDiffuse || bRadius)
						{
							DrawSpriteEx(transitionTexture, 0xFFFFFFFF, 0.0f, 0.0f, outputCX, outputCY, 0.0f, 0.0f, 1.0f, 1.0f);
						}
						else
							DrawSpriteEx(mainRenderTextures[swapIndex], 0xFFFFFFFF, 0.0f, 0.0f, outputCX, outputCY, 0.0f, 0.0f, 1.0f, 1.0f);
					}
					...
					if (!bSameResolut && bUseBack)
					{
						...
						Ortho(0.0f, outputCX_back, outputCY_back, 0.0f, -100.0f, 100.0f);
						SetViewport(0.0f, 0.0f, outputCX_back, outputCY_back);
						...
						if (!bLiveIsHaveFieldSignal)
						{
							if (bTransDisSolving || bTransUpDown || bTransDiffuse || bRadius)
							{
								DrawSpriteEx(transitionTexture, 0xFFFFFFFF, 0.0f, 0.0f, outputCX_back, outputCY_back, 0.0f, 0.0f, 1.0f, 1.0f);
							}
							else
								DrawSpriteEx(mainRenderTextures[swapIndex], 0xFFFFFFFF, 0.0f, 0.0f, outputCX_back, outputCY_back, 0.0f, 0.0f, 1.0f, 1.0f);
						}
						else if (Deinterlacer)
						{
							DrawSpriteEx(Deinterlacer->GetRenderTexture(), 0xFFFFFFFF, 0.0f, 0.0f, outputCX_back, outputCY_back, 0.0f, 0.0f, 1.0f, 1.0f);
						}
					}
					...
					CopyTexture(copyTextures, yuvRenderTextures);
					//profileOut

					if (FAILED(result = Map(copyTextures, lpData, Pitch)))
					{
					...
					if (!bSameResolut && bUseBack)
					{
						...
						CopyTexture(copyTextures_back, yuvRenderTextures_back);
						//profileOut

						if (FAILED(result = Map(copyTextures_back, lpData, Pitch)))
						{
							BUTEL_THORWERROR("Map Failed! ");
						}
			...
			catch (CErrorBase& e)
			{
				if (result == DXGI_ERROR_DEVICE_REMOVED)
				{
					...
					HRESULT reason = GetDeviceRemovedReason();
					...
					{
						CInstanceProcess *Process = m_InstanceList.GetAt(i);
						if (Process && Process->bStartLive)
						{
							GetClientRect(Process->RenderHwnd, &Rect);
							Log::writeMessage(LOG_RTSPSERV, 1, "LiveSDK_Log:%s initD3D Invoke begin! width = %d,heigth = %d", __FUNCTION__, Rect.right, Rect.bottom);
							initD3D(Rect.right, Rect.bottom, Process->RenderHwnd, BSParam.DeviceSetting.AdpterID);
					...
					try
					{
						...
						yuvScalePixelShader = CreatePixelShaderFromFile(TEXT("shaders/DownscaleBilinear1YUV.pShader"));
						...
						transitionPixel = CreatePixelShaderFromFile(TEXT("shaders/SceneTransition.pShader"));
						...
						circleTransitionPixel = CreatePixelShaderFromFile(TEXT("shaders/CircleDiffuse.pShader"));
