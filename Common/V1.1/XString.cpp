...
String::String(LPCSTR str)
{
    ...
    if(curLength)
    {
		lpString = (TSTR)Allocate_Bak((curLength + 1)*sizeof(wchar_t));
    ...
    if(curLength)
    {
		lpString = (TSTR)Allocate_Bak(curLength+1);
        scpy(lpString, str);
    }
    else
        lpString = NULL;
...
String::String(CWSTR str)
{
    ...
    if(curLength)
    {
		lpString = (TSTR)Allocate_Bak((curLength + 1)*sizeof(TCHAR));
        scpy(lpString, str);
    }
    else
        lpString = NULL;
...
Serializer& operator<<(Serializer &s, String &str)
{
    if(s.IsLoading())
    {
        ...
        if(utf8Length)
        {
			LPSTR lpUTF8 = (LPSTR)Allocate_Bak(utf8Length + 1);
...
TCHAR* STDCALL sdup(const TCHAR *str)
{
	...
	TCHAR* lpNewString = (TCHAR*)Allocate_Bak(dataLen);
...
LPSTR tstr_createUTF8(const TCHAR *strSrc)
{
	...
	LPSTR lpString = (LPSTR)Allocate_Bak((UINT)stSize);
...
WSTR tstr_createWide(const TCHAR *strSrc)
{
	...
	WSTR lpString = (WSTR)Allocate_Bak(stSize*sizeof(wchar_t));
 ...
 TSTR utf8_createTstr(const char *strSrc)
{
	...
	TSTR lpString = (TSTR)Allocate_Bak(iSize);
    utf8_to_tchar(strSrc, lpString, iSize/sizeof(wchar_t));
    return lpString;
#else
    int iSize = slen(strSrc)+1;
	LPSTR lpString = (LPSTR)Allocate_Bak(iSize);
