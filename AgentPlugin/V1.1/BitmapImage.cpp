...
void *BI_def_bitmap_create(int width, int height)          { return Allocate_Bak(width * height * 4); }
...
void BitmapImage::CreateErrorTexture(void)
{
    ...
    texture = GetD3DRender()->CreateTexture(32, 32, GS_RGB, textureData, FALSE,TRUE);
...
void BitmapImage::Init(void)
{
    ...
    if(GetPathExtension(lpBitmap).CompareI(TEXT("gif")))
    {
		...
		lpGifData = (LPBYTE)Allocate_Bak(fileSize);
        ...
        if(bIsAnimatedGif)
        {
			...
			texture = GetD3DRender()->CreateTexture(gif.width, gif.height, GS_RGBA, gif.frame_image, FALSE, FALSE);

			animationFrameCache = (BYTE **)Allocate_Bak(gif.frame_count * sizeof(BYTE *));
            memset(animationFrameCache, 0, gif.frame_count * sizeof(BYTE *));

			animationFrameData = (BYTE *)Allocate_Bak(gif.frame_count * gif.width * gif.height * 4);
    ...
    if(!bIsAnimatedGif)
    {
		...
		if (OSFileExists(lpBitmap))
			texture = GetD3DRender()->CreateTextureFromFile(lpBitmap, TRUE, Width, Height);
		else
		{
			Width = 32;
			Height = 32;

			BYTE *textureData = (BYTE*)Allocate_Bak(Width * Height * 4);

			msetd(textureData, 0xFF0000FF, Width * Height * 4);
			texture = GetD3DRender()->CreateTexture(Width, Height, GS_RGBA, textureData, FALSE, TRUE);
			Free(textureData);
		}
        if(!texture)
        {
    ...
    if(bIsAnimatedGif)
    {
        ...
        if(!totalLoops || curLoop < totalLoops)
        {
            ...
            if(newFrame != curFrame)
            {
                ...
                if (animationFrameCache[newFrame])
					GetD3DRender()->SetImage(texture, animationFrameCache[newFrame], GS_IMAGEFORMAT_RGBA, gif.width * 4);
