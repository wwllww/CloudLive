...
{
    UINT val = UINT64(timestamp)*G_LiveInstance->GetAudioEncoder()->GetsampleRate()/1000;
...
class MP4FileStream : public VideoFileStream
{
    ...
    DWORD           lastVideoTimestamp = 0, initialTimeStamp;
    ...
    //chunk stuiff
    UINT64 connectedAudioSampleOffset = 0, connectedVideoSampleOffset = 0;
    UINT64 curVideoChunkOffset = 0, curAudioChunkOffset = 0;
    UINT numVideoSamples = 0, numAudioSamples = 0;
...
public:
	bool ResetFileInit()
	{
		...
		bSentSEI = false;
		videoDecodeTimes.Clear();
		audioDecodeTimes.Clear();
		...
		videoChunks.Clear();
		audioChunks.Clear();
		videoSampleToChunk.Clear();
		audioSampleToChunk.Clear();

		lastVideoTimestamp = 0;
		initialTimeStamp = -1;
		connectedAudioSampleOffset = 0;
		connectedVideoSampleOffset = 0;
		curVideoChunkOffset = 0;
		curAudioChunkOffset = 0;
		numVideoSamples = 0;
		numAudioSamples = 0;
		lastAudioTimeVal = 0;
		audioFrameSize = 0;
		...
		bMP3 = scmp(G_LiveInstance->GetAudioEncoder()->GetCodec(), TEXT("MP3")) == 0;

		audioFrameSize = G_LiveInstance->GetAudioEncoder()->GetFrameSize();
    ...
    void CopyMetadata()
    {
		frameTime = G_LiveInstance->GetFrameTime();
		sampleRateHz = G_LiveInstance->GetAudioEncoder()->GetsampleRate();
		if (m_bBack)
		{
			G_LiveInstance->GetOutputSize_back(width, height);
		}
		else
		{
			G_LiveInstance->GetOutputSize(width, height);
		}

        //-------------------------------------------
        // get AAC headers if using AAC
		maxBitRate = fastHtonl(G_LiveInstance->GetAudioEncoder()->GetBitRate() * 1000);
...
VideoFileStream* CreateMP4FileStream(CTSTR lpFile, bool bBack)
{
