...
void CDeinterlacer::ChangeSize(const DeinterlacerConfig* Config, UINT renderCX, UINT renderCY, DeviceColorType ColorType)
{
	...
	{
		RendTexture = D3DRender->CreateTexture(RenderCX, RenderCY, GS_BGR, NULL, FALSE, FALSE);
		if (deinterlacer.needsPreviousFrame)
			previousTexture = D3DRender->CreateTexture(RenderCX, RenderCY, GS_BGR, NULL, FALSE, FALSE);
		if (deinterlacer.processor == DEINTERLACING_PROCESSOR_GPU)
	...
	{
		RendTexture = D3DRender->CreateTexture(RenderCX, RenderCY, GS_RGB, NULL, FALSE, FALSE);
		if (deinterlacer.needsPreviousFrame)
			previousTexture = D3DRender->CreateTexture(RenderCX, RenderCY, GS_RGB, NULL, FALSE, FALSE);
		if (deinterlacer.processor == DEINTERLACING_PROCESSOR_GPU)
			deinterlacer.texture.reset(D3DRender->CreateRenderTarget(deinterlacer.imageCX, deinterlacer.imageCY, GS_BGRA, FALSE));
	}
}
...
void CDeinterlacer::SetImage(LPVOID clpData, const DeinterlacerConfig* Config, UINT renderCX, UINT renderCY, DeviceColorType colorType)
{
	...
	if (clpData)
	{
		if (colorType == DeviceOutputType_RGB)
		{
			if (RendTexture)
			{
				...
				D3DRender->SetImage(RendTexture, clpData, GS_IMAGEFORMAT_BGRX, linePitch);
				bReadyToDraw = true;
			}
		}
		else if (colorType == DeviceOutputType_I420 || colorType == DeviceOutputType_YV12)
		{
			...
			if (S_OK == D3DRender->Map(RendTexture, lpData, pitch, D3D11_MAP_WRITE_DISCARD))
			{
				PackPlanar(lpData, (LPBYTE)clpData, RenderCX, RenderCY, pitch, 0, RenderCY, linePitch, 0);
				D3DRender->Unmap(RendTexture);
			}
		...
		deinterlacer.pixelShader.get())
	{
		...
		D3DRender->LoadPixelShader(deinterlacer.pixelShader.get());
		...
		D3DRender->DrawSpriteEx(RendTexture, 0xFFFFFFFF, 0.0f, 0.0f, float(deinterlacer.imageCX), float(deinterlacer.imageCY), 0.0f, 0.0f, 1.0f, 1.0f);
	...
	if (RendTexture && bReadyToDraw && deinterlacer.isReady)
	{
		...
		if (deinterlacer.doublesFramerate)
		{
			if (!deinterlacer.curField)
				D3DRender->DrawSpriteEx(tex, 0xFFFFFFFF, pos.x, pos.y, size.x, size.y, 0.f, 0.0f, 0.5f, 1.f);
			else
				D3DRender->DrawSpriteEx(tex, 0xFFFFFFFF, pos.x, pos.y, size.x, size.y, .5f, 0.0f, 1.f, 1.f);
		}
		else
			D3DRender->DrawSprite(tex, 0xFFFFFFFF, pos.x, pos.y, size.x, size.y);
