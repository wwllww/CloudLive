...
void DeviceSource::ChangeSize(bool bSucceeded, bool bForce)
{
    ...
    if(bSucceeded && bUseThreadedConversion)
    {
        ...
        {
            LPBYTE lpData;
			if (S_OK == Map(texture,lpData, texturePitch,D3D11_MAP_WRITE_DISCARD))
				Unmap(texture);
            else
    ...
    if(lastSample)
    {
        ...
        if(colorType == DeviceOutputType_RGB)
        {
            if(texture)
            {
				...
				SetImage(texture,lastSample->lpData, GS_IMAGEFORMAT_BGRX, linePitch);
                bReadyToDraw = true;
            }
        }
        else if(colorType == DeviceOutputType_I420 || colorType == DeviceOutputType_YV12)
        {
            if(bUseThreadedConversion)
            {
                if(!bFirstFrame)
                {
					...
					SetImage(texture,lpImageBuffer, GS_IMAGEFORMAT_RGBX, texturePitch);
...
// 			bSync = (deinterlacer.type == DEINTERLACING_YADIF || deinterlacer.type == DEINTERLACING_YADIF2x);
			...
			bSync ? deinterlacer.pixelShader.get():deinterlacer.pixelShaderAsync.Shader())
        {
			...
			LoadPixelShader(bSync ? deinterlacer.pixelShader.get() : deinterlacer.pixelShaderAsync.Shader());
	...
	if (texture && bReadyToDraw && deinterlacer.isReady)
    {
		...
		if (!FilterTexture)
		{
			...
			if (deinterlacer.doublesFramerate)
			{
				if (!deinterlacer.curField)
					DrawSpriteEx(tex, (opacity255 << 24) | 0xFFFFFF, x, y, x2, y2, 0.f, 0.0f, 0.5f, 1.f);
				else
		...
		else
		{
			...
			if (x > x2 || y > y2)
			{
				if (x > x2 && y > y2)
				{
					DrawSprite(FilterTexture, (opacity255 << 24) | 0xFFFFFF, x2, y2, x, y);
				}
				else if (x > x2)
				{
					DrawSprite(FilterTexture, (opacity255 << 24) | 0xFFFFFF, x2, y, x, y2);
				}
				else
				{
					DrawSprite(FilterTexture, (opacity255 << 24) | 0xFFFFFF, x, y2, x2, y);
				}
			}
			else
			{
				DrawSprite(FilterTexture, (opacity255 << 24) | 0xFFFFFF, x, y, x2, y2);
			}
