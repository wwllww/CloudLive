...
#define WAIT_INFINITE 0xFFFFFFFF
 ...
 OSFileChangeData * STDCALL OSMonitorFileStart(String path, bool suppressLogging = false);
 ...
 String STDCALL OSGetDefaultVideoSavePath(CTSTR append=nullptr);

 BOOL   STDCALL OSCreateMainWindow(int x, int y, int cx, int cy);
 void   STDCALL OSDestroyMainWindow();
 void   STDCALL OSSetWindowSize(int cx, int cy);
 void   STDCALL OSSetWindowFullscreen();
 void   STDCALL OSSetWindowPos(int x, int y);
 ...
 BOOL   STDCALL OSChangeDisplaySettings(DISPLAYMODE *dm);
 void   STDCALL OSGetDisplaySettings(DISPLAYMODE *dm);
 void   STDCALL OSEnumDisplaySettings(List<DISPLAYMODE> &displayModes);
 ...
 void   STDCALL OSSleep100NS(QWORD qw100NSTime); //why
 ...
 BOOL   STDCALL OSTerminateThread(HANDLE hThread, DWORD waitMS=100);
 ...
 QWORD  STDCALL OSGetThreadTime(HANDLE hThread);
...
//  BOOL   STDCALL OSGetLoadedModuleList(HANDLE hProcess, StringList &ModuleList);
 ...
 void __cdecl   LogRaw(const TCHAR *text, UINT len=0);
...
typedef void(*LogUpdateCallback)();
 ...
 void ReadLog(String &data); // do not call this while other threads use any logging functions
 void ReadLogPartial(String &data, unsigned &start, unsigned maxLength=UINT_MAX);
 void ResetLogUpdateCallback(LogUpdateCallback = nullptr);

//-----------------------------------------
//Base functions
//-----------------------------------------
 BOOL STDCALL InitXT(CTSTR logFile=NULL, CTSTR allocatorName=NULL);
 void STDCALL InitXTLog(CTSTR logFile);
 void STDCALL TerminateXT();

 extern BOOL bDebugBreak;
