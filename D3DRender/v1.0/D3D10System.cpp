...
{
    return D3D10Texture::CreateTextureFromFile(lpFile, bBuildMipMaps,Width,Height);
}
...
{
	return D3D10Texture::CreateTextureRead(width, height);
}
...
{
	return D3D10VertexShader::CreateVertexShaderFromFile(lpFileName);
}

Shader* D3D10System::CreatePixelShaderFromFile(CTSTR lpFileName)
{
	return D3D10PixelShader::CreatePixelShaderFromFile(lpFileName);
}
...
HRESULT D3D10System::Map(Texture *tex, BYTE *&lpData, UINT &pitch, D3D11_MAP map_type)
{
	...
	if (FAILED(err = GetD3DCtx()->Map(d3d10Tex->texture, 0, map_type, 0, &map)))
    {
...
{
	D3D10Texture *d3d10Tex = static_cast<D3D10Texture*>(tex);
	GetD3DCtx()->Unmap(d3d10Tex->texture, 0);
}
...
void D3D10System::SetImage(Texture* tex, void *lpData, GSImageFormat imageFormat, UINT pitch)
{
    ...
    HRESULT err;
    D3D11_MAPPED_SUBRESOURCE map;
	if (FAILED(err = GetD3DCtx()->Map(d3d10Tex->texture, 0, D3D11_MAP_WRITE_DISCARD, 0, &map)))
    {
...
{
	return GetD3D()->GetDeviceRemovedReason();
}
