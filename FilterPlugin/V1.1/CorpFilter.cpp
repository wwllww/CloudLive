...
bool FilterCorp::InitFilter(UINT Width, UINT Height)
{
	...
	PixShader = D3DRender->CreatePixelShaderFromFile(TEXT("shaders/Crop_filter.pShader"));
	...
	VertexShader = D3DRender->CreateVertexShaderFromFile(TEXT("shaders/Crop_filter.vShader"));
...
void FilterCorp::FilterRender(Texture *Target, const Vect2 &NewSize)
{
	if (Target && VertexShader && PixShader)
	{
		...
		D3DRender->Ortho(0.0f, width * Mul_val.x, hight * Mul_val.y, 0.0f, -100.0f, 100.0f);
		D3DRender->SetViewport(0, 0, width * Mul_val.x, hight * Mul_val.y);

		D3DRender->DrawSprite(Target, 0xFFFFFFFF, width * Add_val.x, hight * Add_val.y, width * Mul_val.x, hight * Mul_val.y);
	}
}
