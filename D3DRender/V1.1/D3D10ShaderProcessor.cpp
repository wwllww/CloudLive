...
BOOL ShaderProcessor::ProcessShader(CTSTR input, CTSTR filename, D3D10System *D3DSytem)
{
    String curToken;
    ...
    while(GetNextToken(curToken))
    {
        ...
        else if(curToken[0] == '#') //preprocessor
        {
            ...
            if(scmpi_n(curToken, TEXT("include"), 7) == 0)
            {
				...
				ProcessShader(strShader, parent, D3DSytem);
            }
        }
                     ...
                     (curToken != TEXT(";"))         )
            {
                ...
                if(curToken[0] != '(') //verified variable
                {
                    if(strType.CompareI(TEXT("samplerstate")))
                    {
						...
						curSampler.sampler = D3DSytem->CreateSamplerState(info);
