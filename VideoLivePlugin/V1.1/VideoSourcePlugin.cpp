...
#include <windowsx.h>
#include <cstdio>
#define VIDEO_SOURCE_CLASS TEXT("VideoSourceFFmpegZHIBO")
...
bool STDCALL ConfigureVideoSource(Json::Value &element, bool bCreating)
{
	if (bFirst)
	{
		InitProgramListEx(HinstanceDLL);

		pluginLocale = new LocaleStringLookup;

		if (!pluginLocale->LoadStringFile(VIDEOLIVELOCALPATH))
			Log::writeError(LOG_RTSPSERV, 1, "Could not open locale string file '%s'", WcharToAnsi(VIDEOLIVELOCALPATH).c_str());
...
REGINST_CONFIGFUNC(VideoLiveSource, ConfigureVideoSource)
