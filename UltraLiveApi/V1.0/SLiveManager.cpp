		...
		if (NewInstance)
		{
			...
			if (hwnd)
			{
				...
				if (hwnd && !bFirstCreate)
				{
					...
					Texture * Swap = CreateRenderTargetSwapChain((HWND)hwnd, Rect.right, Rect.bottom);
...
void CSLiveManager::SwitchInstanceDisSolve(CInstanceProcess *InstanceS, CInstanceProcess *InstanceD, UINT TransTime)
{
	...
	InstanceD->ClearAudio();
	//InstanceD->FindDifAudioWithInstance(InstanceS);
...
void CSLiveManager::ActuallyEnableProjector()
{
	...
	projectorTexture = CreateRenderTargetSwapChain(hwndProjector, projectorWidth, projectorHeight);
...
void CSLiveManager::RenderSDI(int Index)
{
	...
	if (bStartView && SIDIDs.size())
	{
		...
		if (bEnable)
		{
			...
			CopyTexture(copyRGBTexture, d3dRGB);
			result = Map(copyRGBTexture, lpData, Pitch);
		}
