...
void DelayPushTimeCallBackFunc(bool bShow,const char* ShowTime, bool bClose)
{
    std::string utf;
    ConvertGBKToUtf8(utf,ShowTime);
    QString val(utf.c_str());
    emit g_This->DelayPushTimeChanged(bShow,val,bClose);
}

void NickNameCallBackFunc(uint64_t InstanceID, uint64_t StreamID, const char *NickName)
{
     for(int i = 0; i < g_This->m_VideoInstanceVec.count(); i++)
     {
         if(g_This->m_VideoInstanceVec[i] == InstanceID)
         {
             if(NickName == NULL)
             {
                 emit g_This->NickNamChanged(i,"");
                 return;
             }
             else
             {
                 std::string utf;
                 ConvertGBKToUtf8(utf,NickName);
                 QString val(utf.c_str());
                 emit g_This->NickNamChanged(i,val);
                 return;
             }
         }
     }
...
ButelLive::ButelLive(bool bChat,ChannelInfo* channelInfoList,int channelCount,int currentChannelIndex,QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ButelLive)
{
    ui->setupUi(this);
    g_This = this;
//    CNetInerface::GetInstance()->m_pButelLiveUI = this;
    m_bIsShiftF2 = false;
    m_bIsDelayLive = false;
    ...
    m_pChannelInfo = channelInfoList + currentChannelIndex;

    m_RecFileName = QString(m_pChannelInfo->pChannelName) + '_' + QString::number(m_pChannelInfo->nChannelId) + QString(".cfg");
    ...
    m_PushTimeDelayWnd = NULL;
//    setAttribute(Qt::WA_TranslucentBackground, true);
//    this->setStyleSheet("QWidget#ButelLive{background-color:rgb(35, 35, 37);border-top-left-radius:15px;border-top-right-radius:5px;}");
//    this->setSizeGripEnabled(true);
    m_LiveSettingParam = {0};
    m_bChannelClosed = false;
    ...
    m_bIsNeedRecord = false;
    m_bIsEditing = false;
    m_bPlayLocal = false;
    m_bPlayLive = true;
    m_bSDI = ConfigOper::instance()->m_bSDIOutput;
    m_iSceneNum = 0;
    m_iMateralNum = 0;
    m_iDianboNum = 0;
    m_iHudongNum = 0;
    ...
    connect(this,SIGNAL(DelayPushTimeChanged(bool,QString,bool)),this,SLOT(OnDelayPushTimeChanged(bool,QString,bool)));
    connect(this,SIGNAL(NickNamChanged(int,QString)),this,SLOT(OnNickNamChanged(int,QString)));
    connect(ui->m_pSldLeftCtr,SIGNAL(mouseClicked()),this,SLOT(on_m_pSldLeftCtrsliderReleased()));
    connect(ui->m_pSldRightCtr,SIGNAL(mouseClicked()),this,SLOT(on_m_pSldRightCtrsliderReleased()));
    connect(ui->m_pSldPGMCtr,SIGNAL(mouseClicked()),this,SLOT(on_m_pSldPGMCtrsliderReleased()));
    connect(&m_LiveTimer,SIGNAL(timeout()),this,SLOT(OnLiveTimeOut()));
    connect(&m_SaveSceneTimer,SIGNAL(timeout()),this,SLOT(OnSaveSceneTimeOut()));
//    CNetInerface* netInterface = CNetInerface::GetInstance();
//    netInterface->GetPushStreamServerList(netInterface->m_StrToken);

    m_pChatView = new QWebEngineView(ui->m_pChatScrollArea);
    QWebEnginePage* pViewPage = m_pChatView->page();
    pViewPage->setBackgroundColor(Qt::transparent);

    int iRet = BLiveAppGetSeverNodeList(&m_pServerList,&m_ServerCount);
    char* strAppkey = BLiveAppGetAppkey(ConfigOper::instance()->m_Appkey.toLocal8Bit().data());
    ...
    for(int i = 0; i < 4; i++)
    {
        m_ArrSDIOut[i].Format = NULL;
        m_ArrSDIOut[i].SourceName = NULL;
    }
    ui->m_pSldLeftCtr->installEventFilter(this);
    ui->m_pSldRightCtr->installEventFilter(this);
    ui->m_pSldPGMCtr->installEventFilter(this);
    ui->m_pDisArea->installEventFilter(this);

    m_SaveSceneTimer.start(5*60*1000);
}
...
ButelLive::~ButelLive()
{
    ...
    QString strPath;
    bool exist = dir.exists("./SceneFile");

    if(!exist)
    {
        bool ok = dir.mkdir("./SceneFile");
        if( ok )
        {
            strPath = path + "/SceneFile/";
        }
    }
    else
    {
    ...
    SaveConfig(strPath + "resource.cfg");
    SaveConfig(strPath + m_RecFileName);
    for(int i = 0; i < 4; i++)
    {
        if(m_ArrSDIOut[i].Format)
            free((char*)m_ArrSDIOut[i].Format);
        if(m_ArrSDIOut[i].SourceName)
            free((char*)m_ArrSDIOut[i].SourceName);
    }
    delete ui;
}
...
//初始化直播设定参数
bool ButelLive::InitLiveSettingConfig(const QString& jsonFile)
{
    QFile file(jsonFile);
    ...
    if(jsonError.error == QJsonParseError::NoError)
    {
        if(!(jsonDocument.isNull() || jsonDocument.isEmpty()))
        {
            if(jsonDocument.isObject())
            {
                ...
                if(jsonObj.contains("liveSettingParams"))
                {
                    QJsonValue LiveSettingParam_Val = jsonObj.take("liveSettingParams");
                    if(LiveSettingParam_Val.isObject())
                    {
                        //视频
                        QJsonObject LiveSettingParam_Obj = LiveSettingParam_Val.toObject();
                        QJsonValue Width_value = LiveSettingParam_Obj.take("Width");
                        m_LiveSettingParam.liveSettingParam.Width = Width_value.toInt();
                        QJsonValue Height_value = LiveSettingParam_Obj.take("Height");
                        m_LiveSettingParam.liveSettingParam.Height = Height_value.toInt();
                        QJsonValue VideoBitRate_value = LiveSettingParam_Obj.take("VideoBitRate");
                        m_LiveSettingParam.liveSettingParam.VideoBitRate = VideoBitRate_value.toInt();
                        QJsonValue FPS_value = LiveSettingParam_Obj.take("FPS");
                        m_LiveSettingParam.liveSettingParam.FPS = FPS_value.toInt();
                        QJsonValue bUseHardEncoder_value = LiveSettingParam_Obj.take("bUseHardEncoder");
                        m_LiveSettingParam.liveSettingParam.bUseHardEncoder = bUseHardEncoder_value.toBool();
                        QJsonValue bUseCBR_value = LiveSettingParam_Obj.take("bUseCBR");
                        m_LiveSettingParam.liveSettingParam.bUseCBR = bUseCBR_value.toBool();
                        QJsonValue Quality_value = LiveSettingParam_Obj.take("Quality");
                        m_LiveSettingParam.liveSettingParam.Quality = Quality_value.toInt();
                        if(m_LiveSettingParam.liveSettingParam.bUseCBR)
                        {
                            m_LiveSettingParam.CBRQuality = m_LiveSettingParam.liveSettingParam.Quality;
                        }
                        else
                        {
                            m_LiveSettingParam.VBRQuality = m_LiveSettingParam.liveSettingParam.Quality;
                        }
                        //音频
                        QJsonValue AudioEncoderType_value = LiveSettingParam_Obj.take("AudioEncoderType");
                        m_LiveSettingParam.liveSettingParam.AudioEncoderType = AudioEncoderType_value.toInt();
                        QJsonValue AudioBitRate_value = LiveSettingParam_Obj.take("AudioBitRate");
                        m_LiveSettingParam.liveSettingParam.AudioBitRate = AudioBitRate_value.toInt();
                        QJsonValue AudioSampleRate_value = LiveSettingParam_Obj.take("AudioSampleRate");
                        m_LiveSettingParam.liveSettingParam.AudioSampleRate = AudioSampleRate_value.toInt();
                        QJsonValue AudioChannel_value = LiveSettingParam_Obj.take("AudioChannel");
                        m_LiveSettingParam.liveSettingParam.AudioChannel = AudioChannel_value.toInt();

                        //推流
                        QJsonValue bUsePush_value = LiveSettingParam_Obj.take("bUsePush");
                        m_LiveSettingParam.liveSettingParam.bUsePush = bUsePush_value.toBool();
                        QJsonValue bUseBackPush_value = LiveSettingParam_Obj.take("bUseBackPush");
                        m_LiveSettingParam.liveSettingParam.bUseBackPush = bUseBackPush_value.toBool();
                        QJsonValue LivePushUrl_value = LiveSettingParam_Obj.take("LivePushUrl");
                        strcpy(m_LiveSettingParam.liveSettingParam.LivePushUrl , LivePushUrl_value.toString().toLocal8Bit().data());
                        QJsonValue LiveBackPushUrl_value = LiveSettingParam_Obj.take("LiveBackPushUrl");
                        strcpy(m_LiveSettingParam.liveSettingParam.LiveBackPushUrl , LiveBackPushUrl_value.toString().toLocal8Bit().data());
                        QJsonValue AutoConnect_value = LiveSettingParam_Obj.take("AutoConnect");
                        m_LiveSettingParam.liveSettingParam.AutoConnect = AutoConnect_value.toInt();
                        QJsonValue DelayTime_value = LiveSettingParam_Obj.take("DelayTime");
                        m_LiveSettingParam.liveSettingParam.DelayTime = DelayTime_value.toInt();

                        //录制
                        QJsonValue bRecoder_value = LiveSettingParam_Obj.take("bRecoder");
                        m_LiveSettingParam.liveSettingParam.bRecoder = bRecoder_value.toBool();
                        QJsonValue RecoderPath_value = LiveSettingParam_Obj.take("RecoderPath");
                        strcpy(m_LiveSettingParam.liveSettingParam.RecoderPath , RecoderPath_value.toString().toLocal8Bit().data());
                        QJsonValue FileType_value = LiveSettingParam_Obj.take("FileType");
                        m_LiveSettingParam.liveSettingParam.FileType = FileType_value.toInt();

                        //高级
                        QJsonValue X264Preset_value = LiveSettingParam_Obj.take("X264Preset");
                        strcpy(m_LiveSettingParam.liveSettingParam.X264Preset , X264Preset_value.toString().toLocal8Bit().data());
                        QJsonValue X264Profile_value = LiveSettingParam_Obj.take("X264Profile");
                        strcpy(m_LiveSettingParam.liveSettingParam.X264Profile , X264Profile_value.toString().toLocal8Bit().data());
                        QJsonValue KeyFrame_value = LiveSettingParam_Obj.take("KeyFrame");
                        m_LiveSettingParam.liveSettingParam.KeyFrame = KeyFrame_value.toInt();
                        QJsonValue BFrameCount_value = LiveSettingParam_Obj.take("BFrameCount");
                        m_LiveSettingParam.liveSettingParam.BFrameCount = BFrameCount_value.toInt();

                        //次直播
                        QJsonValue bUseLiveSec_value = LiveSettingParam_Obj.take("bUseLiveSec");
                        m_LiveSettingParam.liveSettingParam.bUseLiveSec = bUseLiveSec_value.toBool();
                        //视频
                        QJsonValue WidthSec_value = LiveSettingParam_Obj.take("WidthSec");
                        m_LiveSettingParam.liveSettingParam.WidthSec = WidthSec_value.toInt();
                        QJsonValue HeightSec_value = LiveSettingParam_Obj.take("HeightSec");
                        m_LiveSettingParam.liveSettingParam.HeightSec = HeightSec_value.toInt();
                        QJsonValue VideoBitRateSec_value = LiveSettingParam_Obj.take("VideoBitRateSec");
                        m_LiveSettingParam.liveSettingParam.VideoBitRateSec = VideoBitRateSec_value.toInt();
                        QJsonValue FPSSec_value = LiveSettingParam_Obj.take("FPSSec");
                        m_LiveSettingParam.liveSettingParam.FPSSec = FPSSec_value.toInt();
                        QJsonValue bUseHardEncoderSec_value = LiveSettingParam_Obj.take("bUseHardEncoderSec");
                        m_LiveSettingParam.liveSettingParam.bUseHardEncoderSec = bUseHardEncoderSec_value.toBool();
                        QJsonValue bUseCBRSec_value = LiveSettingParam_Obj.take("bUseCBRSec");
                        m_LiveSettingParam.liveSettingParam.bUseCBRSec = bUseCBRSec_value.toBool();
                        QJsonValue QualitySec_value = LiveSettingParam_Obj.take("QualitySec");
                        m_LiveSettingParam.liveSettingParam.QualitySec = QualitySec_value.toInt();
                        if(m_LiveSettingParam.liveSettingParam.QualitySec)
                        {
                            m_LiveSettingParam.CBRQualitySec = m_LiveSettingParam.liveSettingParam.QualitySec;
                        }
                        else
                        {
                            m_LiveSettingParam.VBRQualitySec = m_LiveSettingParam.liveSettingParam.QualitySec;
                        }

                        //音频
                        QJsonValue AudioEncoderTypeSec_value = LiveSettingParam_Obj.take("AudioEncoderTypeSec");
                        m_LiveSettingParam.liveSettingParam.AudioEncoderTypeSec = AudioEncoderTypeSec_value.toInt();
                        QJsonValue AudioBitRateSec_value = LiveSettingParam_Obj.take("AudioBitRateSec");
                        m_LiveSettingParam.liveSettingParam.AudioBitRateSec = AudioBitRateSec_value.toInt();
                        QJsonValue AudioSampleRateSec_value = LiveSettingParam_Obj.take("AudioSampleRateSec");
                        m_LiveSettingParam.liveSettingParam.AudioSampleRateSec = AudioSampleRateSec_value.toInt();
                        QJsonValue AudioChannelSec_value = LiveSettingParam_Obj.take("AudioChannelSec");
                        m_LiveSettingParam.liveSettingParam.AudioChannelSec = AudioChannelSec_value.toInt();

                        //推流
                        QJsonValue bUsePushSec_value = LiveSettingParam_Obj.take("bUsePushSec");
                        m_LiveSettingParam.liveSettingParam.bUsePushSec = bUsePushSec_value.toBool();
                        QJsonValue bUseBackPushSec_value = LiveSettingParam_Obj.take("bUseBackPushSec");
                        m_LiveSettingParam.liveSettingParam.bUseBackPushSec = bUseBackPushSec_value.toBool();
                        QJsonValue LivePushUrlSec_value = LiveSettingParam_Obj.take("LivePushUrlSec");
                        strcpy(m_LiveSettingParam.liveSettingParam.LivePushUrlSec , LivePushUrlSec_value.toString().toLocal8Bit().data());
                        QJsonValue LiveBackPushUrlSec_value = LiveSettingParam_Obj.take("LiveBackPushUrlSec");
                        strcpy(m_LiveSettingParam.liveSettingParam.LiveBackPushUrlSec , LiveBackPushUrlSec_value.toString().toLocal8Bit().data());
                        QJsonValue AutoConnectSec_value = LiveSettingParam_Obj.take("AutoConnectSec");
                        m_LiveSettingParam.liveSettingParam.AutoConnectSec = AutoConnectSec_value.toInt();
                        QJsonValue DelayTimeSec_value = LiveSettingParam_Obj.take("DelayTimeSec");
                        m_LiveSettingParam.liveSettingParam.DelayTimeSec = DelayTimeSec_value.toInt();

                        //录制
                        QJsonValue bRecoderSec_value = LiveSettingParam_Obj.take("bRecoderSec");
                        m_LiveSettingParam.liveSettingParam.bRecoderSec = bRecoderSec_value.toBool();
                        QJsonValue RecoderPathSec_value = LiveSettingParam_Obj.take("RecoderPathSec");
                        strcpy(m_LiveSettingParam.liveSettingParam.RecoderPathSec , RecoderPathSec_value.toString().toLocal8Bit().data());
                        QJsonValue FileTypeSec_value = LiveSettingParam_Obj.take("FileTypeSec");
                        m_LiveSettingParam.liveSettingParam.FileTypeSec = FileTypeSec_value.toInt();

                        //高级
                        QJsonValue X264PresetSec_value = LiveSettingParam_Obj.take("X264PresetSec");
                        strcpy(m_LiveSettingParam.liveSettingParam.X264PresetSec , X264PresetSec_value.toString().toLocal8Bit().data());
                        QJsonValue X264ProfileSec_value = LiveSettingParam_Obj.take("X264ProfileSec");
                        strcpy(m_LiveSettingParam.liveSettingParam.X264ProfileSec , X264ProfileSec_value.toString().toLocal8Bit().data());
                        QJsonValue KeyFrameSec_value = LiveSettingParam_Obj.take("KeyFrameSec");
                        m_LiveSettingParam.liveSettingParam.KeyFrameSec = KeyFrameSec_value.toInt();
                        QJsonValue BFrameCountSec_value = LiveSettingParam_Obj.take("BFrameCountSec");
                        m_LiveSettingParam.liveSettingParam.BFrameCountSec = BFrameCountSec_value.toInt();
                    }
                }
            }
        }
    ...
    {
        SetLiveSettingDefault();
    }
    file.close();
    return true;
}
bool ButelLive::InitSDIConfig(const QString& jsonFile)
{
    QFile file(jsonFile);
    if (!file.open(QFile::ReadWrite | QFile::Text)) {
        qDebug() << "Error: Cannot open file: "
    ...
    if(jsonError.error == QJsonParseError::NoError)
    {
        if(!(jsonDocument.isNull() || jsonDocument.isEmpty()))
        {
            ...
            {
                jsonObj = jsonDocument.object();
                if(jsonObj.contains("sdi"))
                {
                    QJsonArray sdiArr = jsonObj.take("sdi").toArray();
                    m_SceneCfgSDIcount = sdiArr.count();
                    for(int i = 0; i < sdiArr.count(); i++)
                    {
                        m_ArrSDIOut[i].Id = sdiArr[i].toObject().take("id").toInt();
                        m_ArrSDIOut[i].bEnable = sdiArr[i].toObject().take("bEnable").toBool();

                        m_ArrSDIOut[i].SourceName = (char*)malloc(strlen(sdiArr[i].toObject().take("SourceName").toString().toLocal8Bit().data()) + 1);
                        m_ArrSDIOut[i].Format = (char*)malloc(strlen(sdiArr[i].toObject().take("Format").toString().toLocal8Bit().data()) + 1);

                        strcpy((char*)m_ArrSDIOut[i].SourceName,sdiArr[i].toObject().take("SourceName").toString().toLocal8Bit().data());
                        strcpy((char*)m_ArrSDIOut[i].Format,sdiArr[i].toObject().take("Format").toString().toLocal8Bit().data());
                    }
                }
            }
        }
...
void ButelLive::SetLiveSettingDefault()
{
    //视频
    m_LiveSettingParam.liveSettingParam.Width = 1280;
    m_LiveSettingParam.liveSettingParam.Height = 720;
    m_LiveSettingParam.liveSettingParam.VideoBitRate = 1200;
    m_LiveSettingParam.liveSettingParam.FPS = 25;
    m_LiveSettingParam.liveSettingParam.bUseHardEncoder = false;
    m_LiveSettingParam.liveSettingParam.bUseCBR = true;
    m_LiveSettingParam.liveSettingParam.Quality = 0;

    //音频
    m_LiveSettingParam.liveSettingParam.AudioEncoderType = 1;
    m_LiveSettingParam.liveSettingParam.AudioBitRate = 96;
    m_LiveSettingParam.liveSettingParam.AudioSampleRate = 44100;
    m_LiveSettingParam.liveSettingParam.AudioChannel = 1;
    //推流
    m_LiveSettingParam.liveSettingParam.bUsePush = true;
    m_LiveSettingParam.liveSettingParam.bUseBackPush = false;
    strcpy(m_LiveSettingParam.liveSettingParam.LivePushUrl,m_pChannelInfo->pPushUrl);
    strcpy(m_LiveSettingParam.liveSettingParam.LiveBackPushUrl,(QString(m_pChannelInfo->pPushUrl) + "_back").toLocal8Bit().data());
    m_LiveSettingParam.liveSettingParam.AutoConnect = 10;
    m_LiveSettingParam.liveSettingParam.DelayTime = 0;
    //录制
    m_LiveSettingParam.liveSettingParam.bRecoder = true;
    strcpy(m_LiveSettingParam.liveSettingParam.RecoderPath,"Videos/.flv");
    m_LiveSettingParam.liveSettingParam. FileType = 1;

    //高级
    strcpy(m_LiveSettingParam.liveSettingParam.X264Preset,"veryfast");
    strcpy(m_LiveSettingParam.liveSettingParam.X264Profile,"high");
    m_LiveSettingParam.liveSettingParam.KeyFrame = 1;
    m_LiveSettingParam.liveSettingParam.BFrameCount = 0;

    //次直播
    m_LiveSettingParam.liveSettingParam. bUseLiveSec = false;
    //视频
    m_LiveSettingParam.liveSettingParam.WidthSec = 1280;
    m_LiveSettingParam.liveSettingParam.HeightSec = 720;
    m_LiveSettingParam.liveSettingParam.VideoBitRateSec = 1200;
    m_LiveSettingParam.liveSettingParam.FPSSec = 25;
    m_LiveSettingParam.liveSettingParam.bUseHardEncoderSec = false;
    m_LiveSettingParam.liveSettingParam.bUseCBRSec = true;
    m_LiveSettingParam.liveSettingParam.QualitySec = 0;

    //音频
    m_LiveSettingParam.liveSettingParam.AudioEncoderTypeSec = 1;
    m_LiveSettingParam.liveSettingParam.AudioBitRateSec = 96;
    m_LiveSettingParam.liveSettingParam.AudioSampleRateSec = 44100;
    m_LiveSettingParam.liveSettingParam.AudioChannelSec = 1;

    //推流
    m_LiveSettingParam.liveSettingParam.bUsePushSec = false;
    m_LiveSettingParam.liveSettingParam.bUseBackPushSec = false;
    strcpy(m_LiveSettingParam.liveSettingParam.LivePushUrlSec,"");
    strcpy(m_LiveSettingParam.liveSettingParam.LiveBackPushUrlSec,"");
    m_LiveSettingParam.liveSettingParam.AutoConnectSec = 10;
    m_LiveSettingParam.liveSettingParam.DelayTimeSec = 0;

    //录制
    m_LiveSettingParam.liveSettingParam.bRecoderSec = false;
    strcpy(m_LiveSettingParam.liveSettingParam.RecoderPathSec,"Videos/.flv");
    m_LiveSettingParam.liveSettingParam. FileTypeSec = 1;

    //高级
    strcpy(m_LiveSettingParam.liveSettingParam.X264PresetSec,"veryfast");
    strcpy(m_LiveSettingParam.liveSettingParam.X264ProfileSec,"high");
    m_LiveSettingParam.liveSettingParam.KeyFrameSec = 1;
    m_LiveSettingParam.liveSettingParam.BFrameCountSec= 0;
}
...
bool ButelLive::InitConfig(const QString& jsonFile)
{
            ...
            if(jsonDocument.isObject())
            {
                ...
                if(jsonObj.contains("materals"))
                {
                    ...
                    for(int i = 0; i < matArr.count(); i++)
                    {
                        ...
                        Log::writeMessage(LOG_RTSPSERV, 1, "%s pMatInfo->type = %d", __FUNCTION__,pMatInfo->type);
                    }
                }
                if(jsonObj.contains("scenes"))
                {
                    ...
                    for(int i = 0; i < scnArr.count(); i++)
                    {
                        ...
                        for(int j = 0; j < linkMatArr.count(); j++)
                        {
                            ...
                            sourceInfo.bKeepRatio = linkMatArr[j].toObject().take("bKeepRatio").toBool();
    ...
    for(int i = 0; i < m_MateralList.count(); i++)
    {
        ...
        Log::writeMessage(LOG_RTSPSERV, 1, "%s iType = %d", __FUNCTION__,iType);
        if(iType == Dianbo || iType == Hudong || iType == Shexiang || iType == Live)
        {
            ...
            else if(iType == Hudong)
            {
                ...
                strJson_obj = strJson_obj.arg(m_pChannelInfo->pInteractionNube).arg(m_pChannelInfo->pAppKey).arg("testname").arg(m_MateralList.at(i)->data);
            }
            else if(iType == Live)
            {
                QJsonArray channelArr;
//                CNetInerface* pNetInstance = CNetInerface::GetInstance();
                ChannelInfo* pChannelInfo = m_channelInfoList ;
                for(int i = 0; i < m_channelCount; i++,pChannelInfo++)
                {
                    QJsonObject channel_item_obj;
                    channel_item_obj.insert("name", m_pChannelInfo->pChannelName);
                    channel_item_obj.insert("flv", m_pChannelInfo->pPlayUrl_flv);
                    channel_item_obj.insert("m3u8", m_pChannelInfo->pPlayUrl_m3u8);
        ...
        {
            strType = tr("ProcTopWindow source");
            m_iProcTopWindowNum++;
        }
    ...
    for(int i = 0; i < ui->m_pLWPreview->count(); i++)
    {
        ...
        Log::writeMessage(LOG_RTSPSERV, 1, "%s m_MateralList[iMatNo]->type = %d", __FUNCTION__,m_MateralList[iMatNo]->type);
        ...
        preItem->m_LbSourceName.setText(preItem->m_LbSourceName.text() + "(" + shortCut + ")");

        QAction* pShortcut = new QAction(this);
        pShortcut->setShortcut(shortCut);
        QObject::connect(pShortcut, &QAction::triggered,[=](bool b){
            preItem->m_BtnToPreview.clicked();
        });
        this->addAction(pShortcut);
...
bool /*ButelLive::*/SaveConfig(const QString& jsonFile)
{
    ...
    if (!file.open(QFile::ReadWrite | QFile::Text | QFile::Truncate)) {
        qDebug() << "Error: Cannot open file: "
                 << qPrintable(file.errorString());
        Log::writeMessage(LOG_RTSPSERV, 1, "%s open file failed!", __FUNCTION__);
        return false;
    }
...
//            linkArr.append(g_This->m_SceneList.at(i)->m_LinkMateralVec.at(j));
            ...
            source_info_obj.insert("bKeepRatio", g_This->m_SceneList.at(i)->m_LinkMateralVec.at(j).bKeepRatio);
    ...
    //写SDI信息
    QJsonArray sdiArr;
    for(int i = 0; i < g_This->m_SDIDeviceVec.count(); i++)
    {
        QJsonObject sdi_item_obj;
        sdi_item_obj.insert("id", g_This->m_ArrSDIOut[i].Id);
        sdi_item_obj.insert("bEnable", g_This->m_ArrSDIOut[i].bEnable);

        std::string utf;
        ConvertGBKToUtf8(utf,g_This->m_ArrSDIOut[i].SourceName);
        QString val(utf.c_str());
        sdi_item_obj.insert("SourceName", val);

        sdi_item_obj.insert("Format", g_This->m_ArrSDIOut[i].Format);
        sdiArr.append(sdi_item_obj);
    }

    //写直播设定信息
    QJsonObject liveSetting_obj;
    //视频
    liveSetting_obj.insert("Width", g_This->m_LiveSettingParam.liveSettingParam.Width);
    liveSetting_obj.insert("Height", g_This->m_LiveSettingParam.liveSettingParam.Height);
    liveSetting_obj.insert("VideoBitRate", g_This->m_LiveSettingParam.liveSettingParam.VideoBitRate);
    liveSetting_obj.insert("FPS", int(g_This->m_LiveSettingParam.liveSettingParam.FPS) );
    liveSetting_obj.insert("bUseHardEncoder", g_This->m_LiveSettingParam.liveSettingParam.bUseHardEncoder);
    liveSetting_obj.insert("bUseCBR", g_This->m_LiveSettingParam.liveSettingParam.bUseCBR);
    liveSetting_obj.insert("Quality", g_This->m_LiveSettingParam.liveSettingParam.Quality);
    //音频
    liveSetting_obj.insert("AudioEncoderType", g_This->m_LiveSettingParam.liveSettingParam.AudioEncoderType);
    liveSetting_obj.insert("AudioBitRate", g_This->m_LiveSettingParam.liveSettingParam.AudioBitRate);
    liveSetting_obj.insert("AudioSampleRate", g_This->m_LiveSettingParam.liveSettingParam.AudioSampleRate);
    liveSetting_obj.insert("AudioChannel", g_This->m_LiveSettingParam.liveSettingParam.AudioChannel);

    //推流
    liveSetting_obj.insert("bUsePush", g_This->m_LiveSettingParam.liveSettingParam.bUsePush);
    liveSetting_obj.insert("bUseBackPush", g_This->m_LiveSettingParam.liveSettingParam.bUseBackPush);
    liveSetting_obj.insert("LivePushUrl", g_This->m_LiveSettingParam.liveSettingParam.LivePushUrl);
    liveSetting_obj.insert("LiveBackPushUrl", g_This->m_LiveSettingParam.liveSettingParam.LiveBackPushUrl);
    liveSetting_obj.insert("AutoConnect", g_This->m_LiveSettingParam.liveSettingParam.AutoConnect);
    liveSetting_obj.insert("DelayTime", g_This->m_LiveSettingParam.liveSettingParam.DelayTime);

    //录制
    liveSetting_obj.insert("bRecoder", g_This->m_LiveSettingParam.liveSettingParam.bRecoder);
    liveSetting_obj.insert("RecoderPath", g_This->m_LiveSettingParam.liveSettingParam.RecoderPath);
    liveSetting_obj.insert("FileType", g_This->m_LiveSettingParam.liveSettingParam.FileType);

    //高级
    liveSetting_obj.insert("X264Preset", g_This->m_LiveSettingParam.liveSettingParam.X264Preset);
    liveSetting_obj.insert("X264Profile", g_This->m_LiveSettingParam.liveSettingParam.X264Profile);
    liveSetting_obj.insert("KeyFrame", g_This->m_LiveSettingParam.liveSettingParam.KeyFrame);
    liveSetting_obj.insert("BFrameCount", g_This->m_LiveSettingParam.liveSettingParam.BFrameCount);

    //次直播
    liveSetting_obj.insert("bUseLiveSec", g_This->m_LiveSettingParam.liveSettingParam.bUseLiveSec);
    //视频
    liveSetting_obj.insert("WidthSec", g_This->m_LiveSettingParam.liveSettingParam.WidthSec);
    liveSetting_obj.insert("HeightSec", g_This->m_LiveSettingParam.liveSettingParam.HeightSec);
    liveSetting_obj.insert("VideoBitRateSec", g_This->m_LiveSettingParam.liveSettingParam.VideoBitRateSec);
    liveSetting_obj.insert("FPSSec",int(g_This->m_LiveSettingParam.liveSettingParam.FPSSec) );
    liveSetting_obj.insert("bUseHardEncoderSec", g_This->m_LiveSettingParam.liveSettingParam.bUseHardEncoderSec);
    liveSetting_obj.insert("bUseCBRSec", g_This->m_LiveSettingParam.liveSettingParam.bUseCBRSec);
    liveSetting_obj.insert("QualitySec", g_This->m_LiveSettingParam.liveSettingParam.QualitySec);
    //音频
    liveSetting_obj.insert("AudioEncoderTypeSec", g_This->m_LiveSettingParam.liveSettingParam.AudioEncoderTypeSec);
    liveSetting_obj.insert("AudioBitRateSec", g_This->m_LiveSettingParam.liveSettingParam.AudioBitRateSec);
    liveSetting_obj.insert("AudioSampleRateSec", g_This->m_LiveSettingParam.liveSettingParam.AudioSampleRateSec);
    liveSetting_obj.insert("AudioChannelSec", g_This->m_LiveSettingParam.liveSettingParam.AudioChannelSec);

    //推流
    liveSetting_obj.insert("bUsePushSec", g_This->m_LiveSettingParam.liveSettingParam.bUsePushSec);
    liveSetting_obj.insert("bUseBackPushSec", g_This->m_LiveSettingParam.liveSettingParam.bUseBackPushSec);
    liveSetting_obj.insert("LivePushUrlSec", g_This->m_LiveSettingParam.liveSettingParam.LivePushUrl);
    liveSetting_obj.insert("LiveBackPushUrlSec", g_This->m_LiveSettingParam.liveSettingParam.LiveBackPushUrlSec);
    liveSetting_obj.insert("AutoConnectSec", g_This->m_LiveSettingParam.liveSettingParam.AutoConnectSec);
    liveSetting_obj.insert("DelayTimeSec", g_This->m_LiveSettingParam.liveSettingParam.DelayTimeSec);

    //录制
    liveSetting_obj.insert("bRecoderSec", g_This->m_LiveSettingParam.liveSettingParam.bRecoderSec);
    liveSetting_obj.insert("RecoderPathSec", g_This->m_LiveSettingParam.liveSettingParam.RecoderPathSec);
    liveSetting_obj.insert("FileTypeSec", g_This->m_LiveSettingParam.liveSettingParam.FileTypeSec);

    //高级
    liveSetting_obj.insert("X264PresetSec", g_This->m_LiveSettingParam.liveSettingParam.X264PresetSec);
    liveSetting_obj.insert("X264ProfileSec", g_This->m_LiveSettingParam.liveSettingParam.X264ProfileSec);
    liveSetting_obj.insert("KeyFrameSec", g_This->m_LiveSettingParam.liveSettingParam.KeyFrameSec);
    liveSetting_obj.insert("BFrameCountSec", g_This->m_LiveSettingParam.liveSettingParam.BFrameCountSec);
    ...
    root_obj.insert("sdi", sdiArr);
    root_obj.insert("liveSettingParams",liveSetting_obj);
...
void ButelLive::UiInit()
{
    ...
    QPixmap pixmap("img/nav_logo.ico");
    QPixmap fitpixmap = pixmap.scaled(ui->m_pTitleBarLabel->m_pIconLabel->width(), ui->m_pTitleBarLabel->m_pIconLabel->height(), Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
    ui->m_pTitleBarLabel->m_pIconLabel->setPixmap(fitpixmap);

    //setWindowIcon(QIcon(":images/nav_logo.png"));
    char* pTitle = BLiveAppGetTitle();
    std::string utf;
    ConvertGBKToUtf8(utf,pTitle);
    QString val(utf.c_str());
    ui->m_pTitleBarLabel->m_pTitleLabel->setText(val + '-' + QString(m_pChannelInfo->pChannelName));
//    ui->m_pTitleBarLabel->m_pIconLabel->setStyleSheet("QLabel{border-image:url(:images/nav_logo.ico);color:#ffffff;font:14pt;}");
    ...
    ui->m_pBtnListen->setStyleSheet("QPushButton"
                                    ...
                                    "}");
    ui->m_pBtnListenPGM->setStyleSheet("QPushButton"
                                    "{"
                                        "border-image:url(:images/tyt_listen.png);"
                                    "}"
                                    "QPushButton:hover"
    ...
    ui->m_pLWPreview->horizontalScrollBar()->setStyleSheet(
                                  "QScrollBar"
                                  "{"
                                    "height:8px;"
                                  "}"
    ...
    ui->m_pLWSceneMateralManage->verticalScrollBar()->setStyleSheet(
                                  "QScrollBar"
                                  "{"
                                    "width:8px;"
                                  "}"
    ...
    ui->m_pBtnReset->setStyleSheet("QPushButton"
                                    ...
                                    "}");
    ui->m_pBtnSDISet->setStyleSheet("QPushButton"
                                    "{"
                                        "border-image:url(:images/SDI_install.png);"
                                    "}"
                                    "QPushButton:hover"
                                    "{"
                                        "border-image:url(:images/SDI_install_hover.png);"
                                    "}");
    if(m_bSDI)
    {
        ui->m_pBtnSDISwitch->setStyleSheet("QPushButton{border-image:url(:images/SDI_on.png);}");
    }
    else
    {
        ui->m_pBtnSDISwitch->setStyleSheet("QPushButton{border-image:url(:images/SDI_off.png);}");
    }
    ...
    QString combStyle = "QComboBox::drop-down"
                        ...
                        "border-radius: 4px; "
                        "}";
    ui->m_pCmbToLive->setStyleSheet(combStyle);
    ui->m_pCmbMonitor->setStyleSheet(combStyle);
    ...
    ui->m_pBtnSDISwitch->setCursor(Qt::PointingHandCursor);
    ui->m_pBtnSDISet->setCursor(Qt::PointingHandCursor);
    ui->m_pBtnScnMtrlAdd->setCursor(Qt::PointingHandCursor);
    ui->m_pBtnScnMtrlDel->setCursor(Qt::PointingHandCursor);
    ui->m_pBtnScnMtrlMoveBottom->setCursor(Qt::PointingHandCursor);
    ui->m_pBtnScnMtrlMoveUp->setCursor(Qt::PointingHandCursor);
    ui->m_pBtnScnMtrlMoveDown->setCursor(Qt::PointingHandCursor);
    ui->m_pBtnScnMtrlMoveTop->setCursor(Qt::PointingHandCursor);
    ui->m_pTitleBarLabel->setCursor(Qt::ArrowCursor);
    ui->m_pWdgtMix->setCursor(Qt::ArrowCursor);
    ui->m_pWndStatus->setCursor(Qt::ArrowCursor);
    ui->m_pLWPreview->setEditTriggers(QAbstractItemView::NoEditTriggers);
    ui->m_pLWPreview->setFocusPolicy(Qt::NoFocus);
    ui->m_pLbLiveTime->hide();
    ui->m_pLbRecordTime->hide();
}
...
void ButelLive::OnConfigResourceInit()
{
    ...
    bool bExist = QFile::exists(strPath + m_RecFileName);
    if(bExist)
    {
        InitConfig(strPath + m_RecFileName);
    }
    else
    {
        InitConfig(strPath + "resource.cfg");
    }
    UpdateRecordState();
...
//    InitSDIConfig("resource.cfg");
    ...
    if(bExist)
    {
        InitSDIConfig(strPath + m_RecFileName);
        InitLiveSettingConfig(strPath + m_RecFileName);
    }
    else
    {
        InitSDIConfig(strPath + "resource.cfg");
        InitLiveSettingConfig(strPath + "resource.cfg");
    }
    int iWidth = m_LiveSettingParam.liveSettingParam.Width;
    int iHeight = m_LiveSettingParam.liveSettingParam.Height;
    int iWidthSec = m_LiveSettingParam.liveSettingParam.WidthSec;
    int iHeightSec = m_LiveSettingParam.liveSettingParam.HeightSec;
    if(m_LiveSettingParam.liveSettingParam.bUseLiveSec)
    {
        if(iWidthSec > iWidth)
        {
            outputCX = m_pixWidth = iWidthSec;
            outputCY = m_pixHeight = iHeightSec;
        }
        else if(iWidthSec == iWidth)
        {
            ...
            {
                outputCX = m_pixWidth = iWidth;
                outputCY = m_pixHeight = iHeight;
            }
        }
        else if(iWidthSec < iWidth)
        {
...
//    m_pixHeight = ConfigOper::instance()->m_Height;
    ...
    m_bArrBlackMagic[0] = ConfigOper::instance()->m_bSDIDevInput1;
    m_bArrBlackMagic[1] = ConfigOper::instance()->m_bSDIDevInput2;
    m_bArrBlackMagic[2] = ConfigOper::instance()->m_bSDIDevInput3;
    m_bArrBlackMagic[3] = ConfigOper::instance()->m_bSDIDevInput4;
...
//	BlackMagicParam       BlackMagic;
    ...
    __SDIOut *pSDIOut = m_ArrSDIOut;

    BlackMagic.bChange = true;
    BlackMagic.bOutSDI = m_bArrBlackMagic;

    LiveSetting.bChange = true;
    LiveSetting.Width = m_LiveSettingParam.liveSettingParam.Width;
    LiveSetting.Height = m_LiveSettingParam.liveSettingParam.Height;
    LiveSetting.FPS = m_LiveSettingParam.liveSettingParam.FPS;
    LiveSetting.VideoBitRate = m_LiveSettingParam.liveSettingParam.VideoBitRate;
    LiveSetting.bUseHardEncoder = m_LiveSettingParam.liveSettingParam.bUseHardEncoder;
    LiveSetting.bUseCBR = m_LiveSettingParam.liveSettingParam.bUseCBR;
    if(LiveSetting.bUseCBR)
    {
        LiveSetting.Quality = m_LiveSettingParam.CBRQuality;
    }
    else
    {
        LiveSetting.Quality = m_LiveSettingParam.VBRQuality;
    }

    LiveSetting.AudioEncoderType = m_LiveSettingParam.liveSettingParam.AudioEncoderType;
    LiveSetting.AudioBitRate = m_LiveSettingParam.liveSettingParam.AudioBitRate;
    LiveSetting.AudioSampleRate = m_LiveSettingParam.liveSettingParam.AudioSampleRate;
    LiveSetting.AudioChannel = m_LiveSettingParam.liveSettingParam.AudioChannel;
    LiveSetting.bUsePush = m_LiveSettingParam.liveSettingParam.bUsePush;
    LiveSetting.bUseBackPush = m_LiveSettingParam.liveSettingParam.bUseBackPush;
    strcpy(LiveSetting.LivePushUrl,m_pChannelInfo->pPushUrl);
//    strcpy(LiveSetting.LiveBackPushUrl,(QString(m_pChannelInfo->pPushUrl) + "_back").toLocal8Bit().data());
    if(strlen(m_LiveSettingParam.liveSettingParam.LiveBackPushUrl) <= 0)
    {
        strcpy(LiveSetting.LiveBackPushUrl,(QString(m_pChannelInfo->pPushUrl) + "_back").toLocal8Bit().data());
    }
    else
    {
        strcpy(LiveSetting.LiveBackPushUrl,m_LiveSettingParam.liveSettingParam.LiveBackPushUrl);
    }
    LiveSetting.AutoConnect = m_LiveSettingParam.liveSettingParam.AutoConnect;
    LiveSetting.DelayTime = m_LiveSettingParam.liveSettingParam.DelayTime;
    LiveSetting.bRecoder = m_LiveSettingParam.liveSettingParam.bRecoder;
    strcpy(LiveSetting.RecoderPath,m_LiveSettingParam.liveSettingParam.RecoderPath);
    LiveSetting.FileType = m_LiveSettingParam.liveSettingParam.FileType;

    strcpy(LiveSetting.X264Preset,m_LiveSettingParam.liveSettingParam.X264Preset);
    strcpy(LiveSetting.X264Profile,m_LiveSettingParam.liveSettingParam.X264Profile);

    LiveSetting.KeyFrame= m_LiveSettingParam.liveSettingParam.KeyFrame;
    LiveSetting.BFrameCount= m_LiveSettingParam.liveSettingParam.BFrameCount;

    LiveSetting.bUseLiveSec = m_LiveSettingParam.liveSettingParam.bUseLiveSec;

    LiveSetting.WidthSec = m_LiveSettingParam.liveSettingParam.WidthSec;
    LiveSetting.HeightSec = m_LiveSettingParam.liveSettingParam.HeightSec;
    LiveSetting.VideoBitRateSec = m_LiveSettingParam.liveSettingParam.VideoBitRateSec;
    LiveSetting.FPSSec = m_LiveSettingParam.liveSettingParam.FPS;
    LiveSetting.bUseHardEncoderSec = m_LiveSettingParam.liveSettingParam.bUseHardEncoderSec;
    LiveSetting.bUseCBRSec = m_LiveSettingParam.liveSettingParam.bUseCBRSec;
    if(LiveSetting.bUseCBRSec)
    {
        LiveSetting.QualitySec = m_LiveSettingParam.CBRQualitySec;
    }
    else
    {
        LiveSetting.QualitySec = m_LiveSettingParam.VBRQualitySec;
    }
    LiveSetting.AudioEncoderTypeSec = m_LiveSettingParam.liveSettingParam.AudioEncoderType;
    LiveSetting.AudioBitRateSec = m_LiveSettingParam.liveSettingParam.AudioBitRate;
    LiveSetting.AudioSampleRateSec = m_LiveSettingParam.liveSettingParam.AudioSampleRate;
    LiveSetting.AudioChannelSec = m_LiveSettingParam.liveSettingParam.AudioChannel;
    LiveSetting.bUsePushSec = m_LiveSettingParam.liveSettingParam.bUsePushSec;
    LiveSetting.bUseBackPushSec = m_LiveSettingParam.liveSettingParam.bUseBackPushSec;
    strcpy(LiveSetting.LivePushUrlSec,m_LiveSettingParam.liveSettingParam.LivePushUrlSec);
    strcpy(LiveSetting.LiveBackPushUrlSec,m_LiveSettingParam.liveSettingParam.LiveBackPushUrlSec);
    LiveSetting.AutoConnectSec = m_LiveSettingParam.liveSettingParam.AutoConnectSec;
    LiveSetting.DelayTimeSec = m_LiveSettingParam.liveSettingParam.DelayTimeSec;
    LiveSetting.bRecoderSec = m_LiveSettingParam.liveSettingParam.bRecoderSec;
    strcpy(LiveSetting.RecoderPathSec,m_LiveSettingParam.liveSettingParam.RecoderPathSec);
    LiveSetting.FileTypeSec = m_LiveSettingParam.liveSettingParam.FileTypeSec;

    //高级
    strcpy(LiveSetting.X264PresetSec,m_LiveSettingParam.liveSettingParam.X264PresetSec);
    strcpy(LiveSetting.X264ProfileSec,m_LiveSettingParam.liveSettingParam.X264ProfileSec);
    LiveSetting.KeyFrameSec= m_LiveSettingParam.liveSettingParam.KeyFrameSec;
    LiveSetting.BFrameCountSec= m_LiveSettingParam.liveSettingParam.BFrameCountSec;

    DeviceSetting.bChange = true;
    DeviceSetting.AdpterID = ConfigOper::instance()->m_AdpterID;
    ...
    else
    {
        ...
        {
           strcpy(DeviceSetting.MonitorDevice,AudioRenderList.at(0).toLocal8Bit().data());
    ...
    else
    {
        if(AudioRenderList.count() > 0)
        {
            strcpy(DeviceSetting.ScrProDevice,AudioRenderList.at(0).toLocal8Bit().data());
    ...
    Advanced.DeinterlaceType = ConfigOper::instance()->m_InterlacedScan;
    ...
    Param.SDICount = m_SceneCfgSDIcount;
    Param.SDIOut = pSDIOut;
    ...
    Param.DelayPushTimeCb = DelayPushTimeCallBackFunc;
    Param.NickNameCb = NickNameCallBackFunc;
    ...
    if(SLiveSetLogLevel(ConfigOper::instance()->m_LogLevel) < 0)
    {
        return;
    }
    InitSDIDeviceList();
    SLiveBlackMagicOutputOn(m_bSDI) ;
    SetAudioPara();
}
...
void ButelLive::on_m_pBtnScene_clicked()
{
    Log::writeMessage(LOG_RTSPSERV, 1, "%s ", __FUNCTION__);
    ui->m_pStckWdgt->setCurrentIndex(0);
    ui->m_pBtnScene->setStyleSheet("QPushButton{border-image:url(:images/btn_scene_materal_press.png);color: rgba(255, 255, 255, 255);}");
    ui->m_pBtnMaterial->setStyleSheet("QPushButton{border-image:url(:images/btn_materal.png);color: rgba(255, 255, 255, 255);}");
    ui->m_pBtnChat->setStyleSheet("QPushButton{border-image:url(:images/btn_materal.png);color: rgba(255, 255, 255, 255);}");
}
...
void ButelLive::on_m_pBtnMaterial_clicked()
{
    ...
    ui->m_pBtnScene->setStyleSheet("QPushButton{border-image:url(:images/btn_scene_materal.png);color: rgba(255, 255, 255, 255);}");
    ui->m_pBtnChat->setStyleSheet("QPushButton{border-image:url(:images/btn_materal.png);color: rgba(255, 255, 255, 255);}");
    ui->m_pBtnMaterial->setStyleSheet("QPushButton{border-image:url(:images/btn_scene_materal_press.png);color: rgba(255, 255, 255, 255);}");
}
...
//    QAction* pActExit = new QAction(QStringLiteral("退出(S)"),this);
    ...
    connect(pActExit,SIGNAL(triggered(bool)),this,SLOT(close()));
...
//    QAction* pLivePreview = new QAction(QStringLiteral("直播预览(L)"),this);
    QAction* pActLive = new QAction(tr("StartLive"),this);
    QAction* pActPreview = new QAction(tr("LocalPreview"),this);
...
//    QAction* pLivePreview = new QAction(tr("live preview(L)"),this);
    ...
    connect(pActLive,SIGNAL(triggered(bool)),this,SLOT(on_m_pBtnStartLive_clicked()));
    connect(pActPreview,SIGNAL(triggered(bool)),this,SLOT(on_m_pBtnLocalPreview_clicked()));

    m_pMenuLive->addSeparator();
    m_pMenuLive->addAction(pActLive);
    m_pMenuLive->addSeparator();
    m_pMenuLive->addAction(pActPreview);
    m_pMenuLive->addSeparator();
...
//    QAction* pCheckUpdate = new QAction(QStringLiteral("检查软件更新"),this);
    ...
    std::string utf;
    ConvertGBKToUtf8(utf,pTitle);
    QString valTitle(utf.c_str());
    valTitle = tr("about%1").arg(valTitle);
    QAction* pAboutBLive = new QAction(valTitle,this);
...
//    m_pMenuAbout->addAction(pDisplayLog);
    ...
    QObject::connect(pAboutBLive, &QAction::triggered,[=](bool b){
        char* pVersion = BLiveAppGetAbout();
        std::string utf;
        ConvertGBKToUtf8(utf,pVersion);
        QString val(utf.c_str());
        AboutUI* pAboutUI = new AboutUI(this,valTitle,val);
        pAboutUI->show();
    });

    QObject::connect(pCheckUpdate, &QAction::triggered,[=](bool b){
        if(CheckProcess(L"UpdateManager.EXE"))
        {
            MyMessageBox * MsgBox = new MyMessageBox(this,tr("information"),tr("The operation cannot be completed because the online upgrade process is running."),BUTTON_YES);
...
//    QAction* pActHelp = new QAction(QStringLiteral("帮助(H)"),this);
    ...
    QAction* pActLogLevel = new QAction(tr("log level"),this);
    ...
    m_MainMenu.addSeparator();
    m_MainMenu.addAction(pActLogLevel);
    m_MainMenu.addSeparator();
    m_MainMenu.addMenu(m_pMenuAbout);
    m_MainMenu.addSeparator();
    m_MainMenu.addAction(pActHelp);

    QObject::connect(pActLogLevel, &QAction::triggered,[=](bool b){

        SetLogLevelUI* pSetLogLevelUI = new SetLogLevelUI(this);
        pSetLogLevelUI->show();
    });
...
//    QMessageBox::information(this,"info","123","OK");
    ...
    if(!m_bIsPreview && !m_bIsLiving && !m_bIsRecording) return;
    int iReserveScene = -1;
    for(int i = 0; i < m_SceneList.count(); i++)
    {
        if(m_SceneList[i]->bIsReserve)
    ...
    if((m_bIsPreview || m_bIsLiving) && m_CurrentSceneNo >= 0 && m_CurrentSceneNo < m_SceneList.count() && iReserveScene >= 0)
    {
...
//        SceneMaterialItem* pSceneMateralItem = new SceneMaterialItem(this,m_MateralList[materalIndex]->name,m_MateralList[materalIndex]->type,ui->m_pLWSceneMateralManage->count());
        disconnect(&pSceneMateralItem->m_CheckBox,SIGNAL(stateChanged(int)),pSceneMateralItem,SLOT(OnCheckBoxClicked(int)));
        ...
        connect(&pSceneMateralItem->m_CheckBox,SIGNAL(stateChanged(int)),pSceneMateralItem,SLOT(OnCheckBoxClicked(int)));
    ...
    if(m_bIsEditing)
    {
        if(m_SelLevel >= 0 && m_SelLevel < m_SceneList[iCurScene]->m_LinkMateralVec.count())
        {
            SetSceneMaterialCurrentRow(ui->m_pLWSceneMateralManage->count() - m_SelLevel - 1);
            SetSceneMaterialItemColor(ui->m_pLWSceneMateralManage->count() - m_SelLevel - 1,true);
        }
    }
}
...
//   if(iIndex < 0) return;
    if(iIndex < 0)
    {
        for(int iItemCount = 0; iItemCount < m_SceneMaterialItemIsSelectedList.count(); iItemCount++)
        {
            if(m_SceneMaterialItemIsSelectedList.at(iItemCount))
            {
                SceneMaterialItem* materialItem = (SceneMaterialItem*)ui->m_pLWSceneMateralManage->itemWidget(ui->m_pLWSceneMateralManage->item(iItemCount));
                setAutoFillBackground(true);
                QPalette palette;
                palette.setColor(QPalette::Base, QColor(35,35,37));
                materialItem->setPalette(palette);
                materialItem->m_CheckBox.setStyleSheet("QCheckBox"
                                                       "{"
                                                       "background-color:rgba(35,35,37,255);"
                                                       "color:rgb(255,255,255);"
                                                       "}"
                                                       "QCheckBox::indicator:unchecked"
                                                       "{"
                                                       "border-image: url(:/images/checkbox_hover.png);"
                                                       "color:rgb(255,255,255);"
                                                       "width:40px;"
                                                       "height:40px;"
                                                       "}"
                                                       ...
                                                       "{"
                                                       "border-image: url(:/images/checkbox_select.png);"
                                                       "color:rgb(255,255,255);"
                                                       "width:40px;"
                                                       "height:40px;"
                                                       "}"
                ...
                materialItem->m_BtnOperator.setStyleSheet("QPushButton{background-color:rgba(35,35,37,255);}"
                                            ...
                                            "QPushButton:hover{border-image:url(:images/scene_edit_hover.png);}"
                                            "QPushButton:pressed{border-image:url(:images/scene_edit_selected.png);}");
                m_SceneMaterialItemIsSelectedList.replace(iItemCount,false);
                break;
            }
        }
        return;
    ...
    else
    {
        ...
        materialItem->m_CheckBox.setStyleSheet("QCheckBox"
                                               "{"
                                               "background-color:rgba(47,160,190,255);"
                                               "color:rgb(255,255,255);"
                                               "}"
                                               "QCheckBox::indicator:unchecked"
                                               "{"
...
void ButelLive::OnImportScene()
{
    ...
    if(!strFileName.isEmpty())
    {
        ...
        //清空主界面除过Shift+F2以外的所有快捷键，在导入场景文件初始化的时候重新加载快捷键
        int iActionCount = this->actions().count();
        while(iActionCount > 0)
        {
            if(this->actions().at(iActionCount - 1)->shortcut().toString() != tr("Shift+F2"))
            {
                this->removeAction(this->actions().at(iActionCount - 1));
            }
            iActionCount--;
        }
        ...
        //如果在场景素材管理界面导入场景文件，初始化场景素材管理项
        if(2 == ui->m_pStckWdgt->currentIndex())
        {
            OnSceneArrowClicked();
        }
        InitSDIDeviceList();
        InitSDIConfig(strFileName);

        int iCount = m_SDIDeviceVec.count();
        __SDIOut* pSDIOut = (__SDIOut*)malloc(iCount * sizeof(__SDIOut));
        __SDIOut* pTmpSDIOut = pSDIOut;
        for(int i = 0; i < iCount; i++,pTmpSDIOut++)
        {
            pTmpSDIOut->Id = m_ArrSDIOut[i].Id;
            pTmpSDIOut->bEnable = m_ArrSDIOut[i].bEnable;
            pTmpSDIOut->SourceName = (char*)malloc(strlen(m_ArrSDIOut[i].SourceName) + 1);
            strcpy((char*)pTmpSDIOut->SourceName,m_ArrSDIOut[i].SourceName);
            pTmpSDIOut->Format = (char*)malloc(strlen(m_ArrSDIOut[i].Format) + 1);
            strcpy((char*)pTmpSDIOut->Format,m_ArrSDIOut[i].Format);
        }
        if(iCount > 0)
        {
...
void ButelLive::OnActSetSceneShortcut()
{
    ...
    int ret = pSetShortcutUI->exec();
    if(ret != ON_BUTTON_OK)
    {
        return;
    }
    QString qstrShortKey = pSetShortcutUI->getInputText();
    if(qstrShortKey.isEmpty() && !m_SceneList[iIndex]->shortcutKeys.isEmpty())
    {
        for(int i = 0; i < this->actions().count(); i++)
        {
    ...
    else if(!qstrShortKey.isEmpty())
    {
        if(!m_SceneList[iIndex]->shortcutKeys.isEmpty())
        {
            for(int i = 0; i < this->actions().count(); i++)
            {
                if(this->actions().at(i)->shortcut().toString() == m_SceneList[iIndex]->shortcutKeys)
                {
                    qDebug() << m_SceneList[iIndex]->shortcutKeys;
                    this->removeAction(this->actions().at(i));
                    break;
                }
            }
...
{
    if(ui->m_pLWPreview->count() >= 12)
    {
        MyMessageBox message(this, tr("error"), tr("The preview item adds up to 12 items"),BUTTON_YES);
        message.exec();
        return;
    }
    Log::writeMessage(LOG_RTSPSERV, 1, "%s ", __FUNCTION__);
...
//            strDShow = strDShow.arg(VideoList.at(0).ShowName).arg(VideoList.at(0).DeviceID).arg(w).arg(h);
    ...
    else
    {
        ...
        //2017.8.9
        QString shortCut = tr("F%1").arg(ui->m_pLWPreview->count());
        preItem->m_LbSourceName.setText(preItem->m_LbSourceName.text() + "(" + shortCut + ")");
...
//        {
            ...
            CPreviewItem* preItem = new CPreviewItem(this,qstrMaterialName,Hudong,ui->m_pLWPreview->count());
            QListWidgetItem* item  = new QListWidgetItem;
            item->setSizeHint(QSize(180,150));
            ui->m_pLWPreview->addItem(item);
            ui->m_pLWPreview->setItemWidget(item,preItem);
            ...
            strJson_obj = strJson_obj.arg(m_pChannelInfo->pInteractionNube).arg(m_pChannelInfo->pAppKey).arg("testname").arg(qstrMaterialName);
            uint64_t StreamID1,StreamID2;
            VideoArea Area;
            Area.left = 0;
            Area.top = 0;
            ...
            else
            {
                ...
                CMaterialItem *MatertialItem = new CMaterialItem(this,qstrMaterialName,tr("Interactive link source"),iMaterialIndex);
                m_pLWMaterialItem = new QListWidgetItem;
                m_pLWMaterialItem->setSizeHint(QSize(0,50));
                ui->m_pLWMaterial->addItem(m_pLWMaterialItem);
                ui->m_pLWMaterial->setItemWidget(m_pLWMaterialItem,MatertialItem);
...
void ButelLive::OnMonitorCapture()
{
    ...
    CMaterialItem *MatertialItem = new CMaterialItem(this,qstrMaterialName,tr("MonitoCapture source"),iMaterialIndex);
    m_pLWMaterialItem = new QListWidgetItem;
    m_pLWMaterialItem->setSizeHint(QSize(0,50));
    ui->m_pLWMaterial->addItem(m_pLWMaterialItem);
    ui->m_pLWMaterial->setItemWidget(m_pLWMaterialItem,MatertialItem);
...
void ButelLive::OnWindowCapture()
{
    ...
    CMaterialItem *MatertialItem = new CMaterialItem(this,qstrMaterialName,tr("WindowCapture source"),iMaterialIndex);
    m_pLWMaterialItem = new QListWidgetItem;
    m_pLWMaterialItem->setSizeHint(QSize(0,50));
    ui->m_pLWMaterial->addItem(m_pLWMaterialItem);
    ui->m_pLWMaterial->setItemWidget(m_pLWMaterialItem,MatertialItem);
...
void ButelLive::OnProcTopWindow()
{
    ...
    CMaterialItem *MatertialItem = new CMaterialItem(this,qstrMaterialName,tr("ProcTopWindowSource source"),iMaterialIndex);
    m_pLWMaterialItem = new QListWidgetItem;
    m_pLWMaterialItem->setSizeHint(QSize(0,50));
    ui->m_pLWMaterial->addItem(m_pLWMaterialItem);
    ui->m_pLWMaterial->setItemWidget(m_pLWMaterialItem,MatertialItem);
...
void ButelLive::OnTextSource()
{
    ...
    CMaterialItem *MatertialItem = new CMaterialItem(this,qstrMaterialName,tr("Text source"),iMaterialIndex);
    m_pLWMaterialItem = new QListWidgetItem;
    m_pLWMaterialItem->setSizeHint(QSize(0,50));
    ui->m_pLWMaterial->addItem(m_pLWMaterialItem);
    ui->m_pLWMaterial->setItemWidget(m_pLWMaterialItem,MatertialItem);
...
//        sourceInfo.cutHeight = m_pixHeight;
        ...
        sourceInfo.bIsAgent = false;
        UINT iTmpW,iTmpH;
        if(SLiveGetStreamSize(tmpInstance,matInfo->streamID1,&iTmpW,&iTmpH) < 0)
        {
            return;
        }
        else
        {
            sourceInfo.width = iTmpW;
            sourceInfo.height = iTmpH;
            sourceInfo.cutWidth = iTmpW;
            sourceInfo.cutHeight = iTmpH;
        }
...
//    CNetInerface* pNetInstance = CNetInerface::GetInstance();
    ...
    {
        QJsonObject channel_item_obj;
        channel_item_obj.insert("name", pChannelInfo->pChannelName);
        channel_item_obj.insert("flv",  pChannelInfo->pPlayUrl_flv);
        channel_item_obj.insert("m3u8", pChannelInfo->pPlayUrl_m3u8);
        channelArr.append(channel_item_obj);
    }
...
void ButelLive::OnDateTimeSource()
{
    ...
    CMaterialItem *MatertialItem = new CMaterialItem(this,qstrMaterialName,tr("Date time source"),iMaterialIndex);
    m_pLWMaterialItem = new QListWidgetItem;
    m_pLWMaterialItem->setSizeHint(QSize(0,50));
    ui->m_pLWMaterial->addItem(m_pLWMaterialItem);
    ui->m_pLWMaterial->setItemWidget(m_pLWMaterialItem,MatertialItem);
...
void ButelLive::OnAudioCatchSource()
{
    ...
    CMaterialItem *MatertialItem = new CMaterialItem(this,qstrMaterialName,tr("Audio capture device"),iMaterialIndex);
    m_pLWMaterialItem = new QListWidgetItem;
    m_pLWMaterialItem->setSizeHint(QSize(0,50));
    ui->m_pLWMaterial->addItem(m_pLWMaterialItem);
    ui->m_pLWMaterial->setItemWidget(m_pLWMaterialItem,MatertialItem);
...
{
    if(index < 0 || index >= ui->m_pLWSceneMateralManage->count())
    {
        SetSceneMaterialItemColor(-1,true);
...
{
    if(-1 == index && 2 == ui->m_pStckWdgt->currentIndex())
    {
        SetSceneMaterialItemColor(ui->m_pLWSceneMateralManage->currentRow(),false);
        ui->m_pLWSceneMateralManage->setCurrentRow(-1);
    }
    int iSel = ui->m_pLWSceneMateralManage->count() - index - 1;
...
void ButelLive::on_m_pBtnDelMaterial_clicked()
{
    ...
    Log::writeMessage(LOG_RTSPSERV, 1, "%s iCurrentRow = %d ", __FUNCTION__,iCurrentRow);
    if(iCurrentRow < 0) return;
    MyMessageBox message(this, tr("information"), tr("Are you sure you want to delete the existing project?"),BUTTON_YES|BUTTON_NO);
    if(message.exec() == ON_BUTTON_OK)
    {
        ...
        Log::writeMessage(LOG_RTSPSERV, 1, "%s SLiveDestroyInstance begin ", __FUNCTION__);
        ...
        Log::writeMessage(LOG_RTSPSERV, 1, "%s m_MateralList.removeAt(%d) ", __FUNCTION__,iCurrentRow);

        //删除小预览区的快捷键
        int iActionCount = this->actions().count();
        while(iActionCount > 0)
        {
            QString strActionShortcut = this->actions().at(iActionCount - 1)->shortcut().toString();
            if(strActionShortcut == tr("F1")  ||
               strActionShortcut == tr("F2")  ||
               strActionShortcut == tr("F3")  ||
               strActionShortcut == tr("F4")  ||
               strActionShortcut == tr("F5")  ||
               strActionShortcut == tr("F6")  ||
               strActionShortcut == tr("F7")  ||
               strActionShortcut == tr("F8")  ||
               strActionShortcut == tr("F9")  ||
               strActionShortcut == tr("F10") ||
               strActionShortcut == tr("F11") ||
               strActionShortcut == tr("F12"))
            {
                this->removeAction(this->actions().at(iActionCount - 1));
            }
            iActionCount--;
        ...
        for(int i = 0; i < ui->m_pLWPreview->count(); i++)
        {
            //2017.8.9
            QString shortCut = tr("F%1").arg(i+1);
            QListWidgetItem * item = ui->m_pLWPreview->item(i);
            CPreviewItem* pPreItem = (CPreviewItem*)ui->m_pLWPreview->itemWidget(item);
            int iMatIndex = GetPreviewSourceLinkMateralNo(i);
            QString strText = m_MateralList[iMatIndex]->name  + "(" + shortCut + ")";
            qDebug() << strText;
            pPreItem->m_LbSourceName.setText(strText);
            QAction* pShortcut = new QAction(this);
            pShortcut->setShortcut(shortCut);
            QObject::connect(pShortcut, &QAction::triggered,[=](bool b){
                pPreItem->m_BtnToPreview.clicked();
            });
            this->addAction(pShortcut);
        }
...
void ButelLive::OnMaterialAddToCurrentScene()
{
    ...
    {
        if(m_SceneList[iCurScene]->m_LinkMateralVec[j].id == iMatIndex && !(m_SceneList[iCurScene]->m_LinkMateralVec[j].bIsAgent))
        {
...
//    if(m_MateralList[iMatIndex]->type == Text || m_MateralList[iMatIndex]->type == Datetime)
//    {
//        UINT iTmpW,iTmpH;
//        if(SLiveGetStreamSize(m_VideoInstanceVec[iMatIndex],m_MateralList[iMatIndex]->streamID1,&iTmpW,&iTmpH) < 0)
//        {
//            return;
//        }
//        else
...
//        }
//    }
    int iType = m_MateralList[iMatIndex]->type;
    if(iType == Datetime)
    {
        sourceInfo.width = 200;
        sourceInfo.height = 48;
    ...
    {
        UINT iTmpW,iTmpH;
        if(SLiveGetStreamSize(m_VideoInstanceVec.at(iMatIndex),m_MateralList[iMatIndex]->streamID1,&iTmpW,&iTmpH) < 0)
        {
            return;
        }
        else
...
void ButelLive::OnMaterialRename()
{
    ...
    else
    {
        ...
        if(m_MateralList[iIndex]->type == Dianbo || m_MateralList[iIndex]->type == Shexiang || m_MateralList[iIndex]->type == Hudong || m_MateralList[iIndex]->type == Live)
        {
            ...
            QString shortCut = tr("F%1").arg(pPreviewItem->m_Index + 1);
            pPreviewItem->m_LbSourceName.setText(qstrSceneName + "(" + shortCut + ")");
        }
    }
}
//素材设置
...
//            ui->m_pLWSceneMateralManage->addItem(m_MateralList.at(i)->name);
            ...
            {
                UINT iTmpW,iTmpH;
                if(SLiveGetStreamSize(m_VideoInstanceVec.at(i),m_MateralList[i]->streamID1,&iTmpW,&iTmpH) < 0)
                {
                    return;
                }
                else
...
//            if(sceneInfo->m_LinkMateralVec[i].bRender)
        ...
        m_pMenuLive->actions().at(3)->setText(tr("StopPreview"));
        m_pMenuLive->actions().at(1)->setEnabled(false);
        ui->m_pBtnStartLive->setEnabled(false);
        ui->m_pBtnStartLive->setStyleSheet("QPushButton{color: rgba(128, 128, 128, 255);background-color: rgba(57, 57, 59, 0);}");
        ui->m_pBtnRecord->setEnabled(false);
        ui->m_pBtnRecord->setStyleSheet("QPushButton{color: rgba(128, 128, 128, 255);background-color: rgba(57, 57, 59, 0);}");
        ui->m_pLbRecord->setStyleSheet("QLabel{border-image:url(:images/video_rec_unselect.png);}");
    ...
    else
    {
        ...
        m_pMenuLive->actions().at(3)->setText(tr("LocalPreview"));
        if(!m_bChannelClosed)
        {
            m_pMenuLive->actions().at(1)->setEnabled(true);
            ui->m_pBtnStartLive->setEnabled(true);
            ui->m_pBtnStartLive->setStyleSheet("QPushButton{color: rgba(255, 255, 255, 255);background-color: rgba(57, 57, 59, 0);}");
        }

        ui->m_pBtnEditMode->setEnabled(false);
        ui->m_pBtnEditMode->setStyleSheet("QPushButton{color: rgba(128, 128, 128, 255);background-color: rgba(57, 57, 59, 0);}");
        if(m_bIsEditing)
        {
            if(m_SelLevel >= 0)
            {
...
void ButelLive::UpdateEdge()
{
    ...
    {
        ui->gridLayout_5->setContentsMargins(5,2,5,5);
    }
...
void ButelLive::on_m_pLWScene_currentRowChanged(int currentRow)
{
    ...
    m_SelLevel = -1;
    Log::writeMessage(LOG_RTSPSERV, 1, "%s ", __FUNCTION__);
    if(!m_bIsPreview && !m_bIsLiving && !m_bIsRecording)
    {
        Log::writeMessage(LOG_RTSPSERV, 1, "%s  currentRow = %d", __FUNCTION__,currentRow);
    ...
    if((m_bIsPreview || m_bIsLiving || m_bIsRecording) && m_CurrentSceneNo >= 0 && m_CurrentSceneNo < m_SceneList.count() && currentRow >= 0)
    {        
        //开始预览现在场景中源
...
//            if(sceneInfo->m_LinkMateralVec[i].bRender)
    ...
    else if((m_bIsPreview || m_bIsLiving || m_bIsRecording) && m_CurrentSceneNo < 0 && currentRow >= 0 && currentRow >= 0)
    {
        //开始预览现在场景中源
        SceneInfo * sceneInfo = m_SceneList.at(currentRow);
        for(int i = 0; i < sceneInfo->m_LinkMateralVec.count(); i++)
...
//    int iCurScnMat = ui->m_pLWSceneMateralManage->count() - index - 1;
    ...
    if(message.exec() == ON_BUTTON_OK)
    {
        ...
        if(m_bIsPreview || m_bIsLiving)
        {
            ...
            {
                Log::writeMessage(LOG_RTSPSERV, 1, "%s SLiveDelStream failed!", __FUNCTION__);
                return;
            }
        }
        if(bIsAgent)
        ...
        if(2 == ui->m_pStckWdgt->currentIndex())
        {
            ...
            int index = ui->m_pLWSceneMateralManage->currentRow();
            if(index < 0) return;
            Log::writeMessage(LOG_RTSPSERV, 1, "%s index = %d", __FUNCTION__,index);
...
void ButelLive::InitSDIDeviceList()
{
    char* DevicesList;
    m_SDIDeviceVec.clear();
    if(0 == SLiveGetBlackMagicDevices(&DevicesList))
    {
        std::string utf;
    ...
    m_ModeList.clear();
    if(0 == SLiveGetBlackMagicDisplayMode(&DisplayModeList))
    {
        std::string utf;
...
{
    Log::writeMessage(LOG_RTSPSERV, 1, "%s iMatNo = %d,bIsAgent = %d", __FUNCTION__,iMatNo,bIsAgent);
    int iType;
    ...
    if(bIsAgent)
    {
        Log::writeMessage(LOG_RTSPSERV, 1, "%s bIsAgent = true", __FUNCTION__);
    ...
    {
        Log::writeMessage(LOG_RTSPSERV, 1, "%s bIsAgent = false", __FUNCTION__);
        ...
        for(int i = 0; i < m_SceneList[iCurScene]->m_LinkMateralVec.count(); i++)
        {
            ...
            if(!bIsAgent)
            {
                if(m_SceneList[iCurScene]->m_LinkMateralVec[i].id == iMatNo)
                {
                    Log::writeMessage(LOG_RTSPSERV, 1, "%s iMatNo = %d ", __FUNCTION__,iMatNo);
                    streamW = m_SceneList[iCurScene]->m_LinkMateralVec[i].width;
                    streamH = m_SceneList[iCurScene]->m_LinkMateralVec[i].height;
                    left = m_SceneList[iCurScene]->m_LinkMateralVec[i].left;
                    top = m_SceneList[iCurScene]->m_LinkMateralVec[i].top;
    ...
    {
        Log::writeMessage(LOG_RTSPSERV, 1, "%s AgentSource ", __FUNCTION__);
        if(SLiveAdd2Intance(m_AgentInstanceVec.at(iMatNo),Instance,&AreaCut,bIsRender) < 0)
        {
            return;
        }
    }
    ...
    {
        Log::writeMessage(LOG_RTSPSERV, 1, "%s !AgentSource ", __FUNCTION__);
        if(SLiveAdd2Intance(m_VideoInstanceVec.at(iMatNo),Instance,&AreaCut,bIsRender) < 0)
        {
            return;
        }
    }
...
void ButelLive::on_m_pBtnStartLive_clicked()
{
    ...
    if(m_bIsLiving)
    {
//        StopLiveConfirmUI * pStopLiveUI = new StopLiveConfirmUI(this);
//        int ret = pStopLiveUI->exec();
//        if(ON_BUTTON_OK == ret)
        int ret = BLiveAppBeforeStopPushConfirm(m_bChannelClosed);
        if(0 == ret)
        {
            ...
            else
            {
                ...
                m_pMenuLive->actions().at(1)->setText(tr("start live"));
                m_pMenuLive->actions().at(3)->setEnabled(true);
                ui->m_pBtnLocalPreview->setEnabled(true);

                if(!m_bChannelClosed)
//                    ui->m_pBtnStartLive->setStyleSheet("QPushButton{color: rgba(255, 255, 255, 255);background-color: rgba(57, 57, 59, 0);}");
                    SetStartLiveEnable(true);
                else
                    SetStartLiveEnable(false);

                ui->m_pBtnLocalPreview->setStyleSheet("QPushButton{color: rgba(255, 255, 255, 255);background-color: rgba(57, 57, 59, 0);}");


                bool bRecord;
                if(SLiveHasIntancesCanRecord(&bRecord) < 0)
                {
                    return;
                }
                else
                {
                    if(bRecord)
                    {
                        ui->m_pBtnRecord->setEnabled(true);
                        ui->m_pBtnRecord->setStyleSheet("QPushButton{color: rgb(255, 255, 255);background-color: rgba(57, 57, 59, 0);}");
                    }
                    else
                    {
                        ui->m_pBtnRecord->setEnabled(false);
                        ui->m_pBtnRecord->setStyleSheet("QPushButton{color: rgb(128, 128, 128);background-color: rgba(57, 57, 59, 0);}");
                    }
                    ui->m_pLbRecord->setStyleSheet("QLabel{border-image:url(:images/video_rec_unselect.png);}");
                }
                ...
                m_LiveTimer.stop();
                ui->m_pLbLiveTime->setText("00:00:00");
                ui->m_pLbLiveTime->hide();
                BLiveAppAfterStopPush();
            }
        }
    }
    else
    {
        m_bIsNeedRecord = false;
        MessageInfoUI* pMessageInfoUi = new MessageInfoUI(this);
        pMessageInfoUi->SetInfo(tr("The live broadcast is being launched. Please wait a moment..."));
        pMessageInfoUi->show();
        int ret = BLiveAppBeforeStartPush();
        if(0 == ret)
        {
            pMessageInfoUi->close();
            StartLive();
            char* strChatUrl = BLiveAppGetChatUrl();
            qDebug() << strChatUrl;
            m_pChatView->setUrl(QUrl(strChatUrl));
        }
        else
        {
            pMessageInfoUi->SetInfo(tr("Failed to launch live broadcast"));
            pMessageInfoUi->SetOKBtnShow(true);
        }
    }
}
void ButelLive::StartLive()
{
    if (SLiveStartLive(Instance_2,m_bIsNeedRecord) < 0)
    {
        return;
    }
    else
    {
        ...
        if(m_bIsNeedRecord)
        {
            ui->m_pBtnRecord->setText(tr("StopRecord"));
            ui->m_pLbRecordTime->show();
            m_bIsRecording = true;
            m_pMenuLive->actions().at(1)->setEnabled(false);
            ui->m_pBtnStartLive->setEnabled(false);
            ui->m_pBtnStartLive->setStyleSheet("QPushButton{color: rgba(128, 128, 128, 255);background-color: rgba(57, 57, 59, 0);}");
            ui->m_pBtnRecord->setStyleSheet("QPushButton{color: rgb(192, 42, 66);background-color: rgba(57, 57, 59, 0);}");
            ui->m_pLbRecord->setStyleSheet("QLabel{border-image:url(:images/video_rec_selected.png);}");
        }
        else
        {
            ui->m_pLbLiveTime->show();
            m_bIsLiving = true;
            ...
            m_pMenuLive->actions().at(1)->setText(tr("stop live"));
            ui->m_pBtnStartLive->setStyleSheet("QPushButton{color: rgba(0, 170, 255, 255);background-color: rgba(57, 57, 59, 0);}");
        }

        m_pMenuLive->actions().at(3)->setEnabled(false);
        ui->m_pBtnLocalPreview->setEnabled(false);     
        ui->m_pBtnLocalPreview->setStyleSheet("QPushButton{color: rgba(128, 128, 128, 255);background-color: rgba(57, 57, 59, 0);}");
...
void ButelLive::on_m_pBtnEditMode_clicked()
{
    ...
    if(!m_bIsEditing)
    {
        if(m_SceneList[iCurScene]->m_LinkMateralVec.count() <= 0) return;
        if(2 == ui->m_pStckWdgt->currentIndex())
        {
            if(ui->m_pLWSceneMateralManage->currentRow() < 0)
            {
                m_SelLevel = m_SceneList[iCurScene]->m_LinkMateralVec.count() - 1;
                SetSceneMaterialItemColor(0,true);
                SetSceneMaterialCurrentRow(0);
            }
            else
            {
                m_SelLevel = ui->m_pLWSceneMateralManage->count() - ui->m_pLWSceneMateralManage->currentRow() - 1;
            }
        }
        else
        {
...
//    int iCurScnMat = ui->m_pLWSceneMateralManage->count() - index - 1;
    ...
    bool bKeepRatio = m_SceneList[GetSceneCurrentRow()]->m_LinkMateralVec[iCurScnMat].bKeepRatio;
    if(bIsAgent)
    {
    ...
    {        
        ...
        else
        {
            ...
            else if(m_MateralList[iLinkMatIndex]->type == Text)
            {
                UINT iTmpW,iTmpH;
                if(SLiveGetStreamSize(m_VideoInstanceVec.at(iLinkMatIndex),m_MateralList.at(iLinkMatIndex)->streamID1,&iTmpW,&iTmpH) < 0)
                {
                    return;
                }
                else
                {
                    int iCurScene = GetSceneCurrentRow();
                    m_SceneList[iCurScene]->m_LinkMateralVec[index].width = iTmpW;
                    m_SceneList[iCurScene]->m_LinkMateralVec[index].height = iTmpH;
                    m_SceneList[iCurScene]->m_LinkMateralVec[index].cutWidth = iTmpW;
                    m_SceneList[iCurScene]->m_LinkMateralVec[index].cutHeight = iTmpH;
                    ...
                    if(m_bIsPreview || m_bIsLiving)
                    {
                        ...
                        Area.CropRight = 0;
                        Area.CropBottom = 0;
                        if(SLiveUpdateStreamPosition(Instance,m_MateralList.at(iLinkMatIndex)->streamID1,&Area,/*ConfigOper::instance()->m_bKeepRatio*/bKeepRatio) < 0)
                        {
                            return;
                        }
                        else
...
void ButelLive::on_m_pBtnScnMtrlMoveDown_clicked()
{
    ...
    {
        int iIndex = GetSceneMaterialCurrentRow();
        if(iIndex < 0 ||iIndex >=  ui->m_pLWSceneMateralManage->count()-1) return;
        disconnect(ui->m_pLWSceneMateralManage,SIGNAL(currentRowChanged(int)),this,SLOT(on_m_pLWSceneMateralManage_currentRowChanged(int)));
...
//    int index = ui->m_pLWSceneMateralManage->count() - GetSceneMaterialCurrentRow() - 1;
    ...
    bool bKeepRatio = m_SceneList[iCurScene]->m_LinkMateralVec[index].bKeepRatio;
    VideoArea Area, AreaCut;
...
//    float fScal = Area.width/Area.height;
    UINT streamWidth,streamHeight;
    if(bIsAgent)
    {
        if(SLiveGetStreamSize(Instance,m_AgentSourceList[iMatIndex]->streamID1,&streamWidth,&streamHeight) < 0)
        {
            return;
        }
    }
    ...
    {
        if(SLiveGetStreamSize(Instance,m_MateralList[iMatIndex]->streamID1,&streamWidth,&streamHeight) < 0)
        {
            return;
        }
    }
    float fScal = (float)streamWidth/streamHeight;
    float fScalStandard = (float)m_pixWidth/m_pixHeight;

    Log::writeMessage(LOG_RTSPSERV, 1, "%s m_pixWidth = %d,m_pixHeight = %d,fScal = %f,fScalStandard = %f", __FUNCTION__,m_pixWidth,m_pixHeight,fScal,fScalStandard);
    if(fScal > fScalStandard)
    {
        Area.width = m_pixWidth;
        Area.height = Area.width / fScal;
        Area.left = 0;
        //Area.top = m_SceneList[iCurScene]->m_LinkMateralVec[index].top - (Area.height - m_SceneList[iCurScene]->m_LinkMateralVec[index].height) / 2;
        Area.top = (m_pixHeight - Area.height) / 2;
    }
    else if(fScal < fScalStandard)
    {
        Area.height = m_pixHeight;
        Area.width = Area.height * fScal;
        Area.top = 0;
        //Area.left = m_SceneList[iCurScene]->m_LinkMateralVec[index].left - (Area.width - m_SceneList[iCurScene]->m_LinkMateralVec[index].width) / 2;
        Area.left = (m_pixWidth - Area.width) / 2;
    }
    else
    {
        Area.height = m_pixHeight;
    ...
    AreaCut.width = m_SceneList[iCurScene]->m_LinkMateralVec[index].cutWidth;
    AreaCut.height = m_SceneList[iCurScene]->m_LinkMateralVec[index].cutHeight;

//    AreaCut.width = Area.width / m_SceneList[iCurScene]->m_LinkMateralVec[index].width * AreaCut.width;
//    AreaCut.height = Area.height / m_SceneList[iCurScene]->m_LinkMateralVec[index].height * AreaCut.height;

    AreaCut.CropLeft = m_SceneList[iCurScene]->m_LinkMateralVec[index].left - m_SceneList[iCurScene]->m_LinkMateralVec[index].cutLeft;
    AreaCut.CropTop = m_SceneList[iCurScene]->m_LinkMateralVec[index].top - m_SceneList[iCurScene]->m_LinkMateralVec[index].cutTop;
    AreaCut.CropRight = m_SceneList[iCurScene]->m_LinkMateralVec[index].cutWidth - m_SceneList[iCurScene]->m_LinkMateralVec[index].width - AreaCut.CropLeft;
    AreaCut.CropBottom = m_SceneList[iCurScene]->m_LinkMateralVec[index].cutHeight - m_SceneList[iCurScene]->m_LinkMateralVec[index].height - AreaCut.CropTop;


    AreaCut.CropLeft = Area.width / m_SceneList[iCurScene]->m_LinkMateralVec[index].width * AreaCut.CropLeft;
    AreaCut.CropTop = Area.height / m_SceneList[iCurScene]->m_LinkMateralVec[index].height * AreaCut.CropTop;
    AreaCut.CropRight = Area.width / m_SceneList[iCurScene]->m_LinkMateralVec[index].width * AreaCut.CropRight;
    AreaCut.CropBottom = Area.height / m_SceneList[iCurScene]->m_LinkMateralVec[index].height * AreaCut.CropBottom;
    AreaCut.width = Area.width + AreaCut.CropLeft + AreaCut.CropRight;
    AreaCut.height = Area.height + AreaCut.CropTop + AreaCut.CropBottom;

    AreaCut.left = Area.left - AreaCut.CropLeft;
    AreaCut.top = Area.top - AreaCut.CropTop;

    if(bIsAgent)
    {
        if(SLiveUpdateStreamPosition(Instance,m_AgentSourceList[iMatIndex]->streamID1,&AreaCut,/*ConfigOper::instance()->m_bKeepRatio*/bKeepRatio) < 0)
        {
            return;
        }
        else
        {
            if(0 == SLiveSetCropping(Instance,m_AgentSourceList[iMatIndex]->streamID1,AreaCut.CropLeft,AreaCut.CropTop,AreaCut.CropRight,AreaCut.CropBottom))
            {
                m_SceneList[iCurScene]->m_LinkMateralVec[index].left = Area.left;
                m_SceneList[iCurScene]->m_LinkMateralVec[index].top = Area.top;
                m_SceneList[iCurScene]->m_LinkMateralVec[index].cutLeft = AreaCut.left;
                m_SceneList[iCurScene]->m_LinkMateralVec[index].cutTop = AreaCut.top;
                m_SceneList[iCurScene]->m_LinkMateralVec[index].width = Area.width;
                m_SceneList[iCurScene]->m_LinkMateralVec[index].height = Area.height;
                m_SceneList[iCurScene]->m_LinkMateralVec[index].cutWidth = AreaCut.width;
                m_SceneList[iCurScene]->m_LinkMateralVec[index].cutHeight = AreaCut.height;
            }
    ...
    {
        if(SLiveUpdateStreamPosition(Instance,m_MateralList[iMatIndex]->streamID1,&AreaCut,/*ConfigOper::instance()->m_bKeepRatio*/bKeepRatio) < 0)
        {
            return;
        }
        else
        {
            if(0 == SLiveSetCropping(Instance,m_MateralList[iMatIndex]->streamID1,AreaCut.CropLeft,AreaCut.CropTop,AreaCut.CropRight,AreaCut.CropBottom))
            {
                m_SceneList[iCurScene]->m_LinkMateralVec[index].left = Area.left;
                m_SceneList[iCurScene]->m_LinkMateralVec[index].top = Area.top;
                m_SceneList[iCurScene]->m_LinkMateralVec[index].cutLeft = AreaCut.left;
...
void ButelLive::OnActResetSize()
{
    ...
    if(bIsAgent)
    {
        if(SLiveGetStreamSize(Instance,m_AgentSourceList[iMatIndex]->streamID1,&width,&height) < 0)
        {
            return;
        }
    }
    ...
    {
        if(SLiveGetStreamSize(Instance,m_MateralList[iMatIndex]->streamID1,&width,&height) < 0)
        {
            return;
        }
    }
    ...
    float fScal = Area.width/Area.height;
    float fScalStandard = width/height;
...
//    int index = ui->m_pLWSceneMateralManage->count() - GetSceneMaterialCurrentRow() - 1;
    int index = m_SelLevel;
    VideoArea AreaCut;
    int left = m_SceneList[iCurScene]->m_LinkMateralVec[index].left;
    int top = m_SceneList[iCurScene]->m_LinkMateralVec[index].top;
    int streamW = m_SceneList[iCurScene]->m_LinkMateralVec[index].width;
    int streamH = m_SceneList[iCurScene]->m_LinkMateralVec[index].height;
    int cutLeft = m_SceneList[iCurScene]->m_LinkMateralVec[index].cutLeft;
    int cutTop = m_SceneList[iCurScene]->m_LinkMateralVec[index].cutTop;
    int streamCutW = m_SceneList[iCurScene]->m_LinkMateralVec[index].cutWidth;
    int streamCutH = m_SceneList[iCurScene]->m_LinkMateralVec[index].cutHeight;
    AreaCut.left = cutLeft;
    AreaCut.top = cutTop;
    AreaCut.width = streamCutW;
    AreaCut.height = streamCutH;
    ...
    AreaCut.CropRight = (streamCutW-streamW)-(left-cutLeft);
    AreaCut.CropBottom = (streamCutH-streamH)-(top-cutTop);
    PositionSizeSetUI* pPosSizeSetUi = new PositionSizeSetUI(this,&AreaCut);
    pPosSizeSetUi->show();
}
void ButelLive::OnActResetCut()
{
    ...
    if(bIsAgent)
    {
        if(0 == SLiveSetCropping(Instance,m_AgentSourceList[iMatIndex]->streamID1,0,0,0,0))
        {
            m_SceneList[iCurScene]->m_LinkMateralVec[index].left = cutLeft;
            m_SceneList[iCurScene]->m_LinkMateralVec[index].top = cutTop;
            m_SceneList[iCurScene]->m_LinkMateralVec[index].width = streamCutW;
            m_SceneList[iCurScene]->m_LinkMateralVec[index].height = streamCutH;
            m_SceneList[iCurScene]->m_LinkMateralVec[index].bIsCut = false;
        }
    }
    else
    {
        if(0 == SLiveSetCropping(Instance,m_MateralList[iMatIndex]->streamID1,0,0,0,0))
        {
            m_SceneList[iCurScene]->m_LinkMateralVec[index].left = cutLeft;
            m_SceneList[iCurScene]->m_LinkMateralVec[index].top = cutTop;
            m_SceneList[iCurScene]->m_LinkMateralVec[index].width = streamCutW;
...
//    int index = ui->m_pLWSceneMateralManage->count() - GetSceneMaterialCurrentRow() - 1;
    ...
    AreaCut.CropLeft = m_SceneList[iCurScene]->m_LinkMateralVec[index].left - m_SceneList[iCurScene]->m_LinkMateralVec[index].cutLeft;
    AreaCut.CropTop = m_SceneList[iCurScene]->m_LinkMateralVec[index].top - m_SceneList[iCurScene]->m_LinkMateralVec[index].cutTop;
    AreaCut.CropRight = AreaCut.width - Area.width - AreaCut.CropLeft;
    AreaCut.CropBottom = AreaCut.height - Area.height - AreaCut.CropTop;
    AreaCut.left = Area.left - AreaCut.CropLeft;
    AreaCut.top = Area.top - AreaCut.CropTop;
...
void ButelLive::OnPreiveFullScreen()
{
    if(!m_bIsFullScreenPreview)
    {
        ...
        m_PreviewMenu.actions().at(0)->setCheckable(true);
    }
    else
    {
        ...
        SLiveStartResize(Instance,false);
        ResizeRenderFrame();
        m_PreviewMenu.actions().at(0)->setCheckable(false);
        SetFocus(HWND(this->winId()));
    }
}
...
void ButelLive::OnDelayPushTimeChanged(bool bShow,QString ShowTime, bool bClose)
{
    if(bShow)
    {
        m_PushTimeDelayWnd = new PushStreamDelayUI(this);
        m_PushTimeDelayWnd->show();
    }
    else
    {
        if(!ShowTime.isEmpty())
        {
            m_PushTimeDelayWnd->SetInfo(ShowTime);
        }
    }
    if(bClose)
    {
        if(m_PushTimeDelayWnd)
        {
            m_PushTimeDelayWnd->close();
            m_PushTimeDelayWnd = NULL;
        }
        if(m_bIsDelayLive)
        {
...
void ButelLive::OnNickNamChanged(int InstanceIndex, QString Name)
{
    int iPreIndex = GetMateralSourceLinkPreviewNo(InstanceIndex);
    if(iPreIndex < 0) return;
    CPreviewItem* pPreviewItem = (CPreviewItem*)ui->m_pLWPreview->itemWidget(ui->m_pLWPreview->item(iPreIndex));
    QString strShortCut = tr("F%1").arg(pPreviewItem->m_Index + 1);
    if(Name.isEmpty())
    {
        pPreviewItem->m_LbSourceName.setText(m_MateralList[InstanceIndex]->name + "(" +strShortCut + ")");
    }
    else
    {
        pPreviewItem->m_LbSourceName.setText(Name + "("  + strShortCut+ ")");
    }
}

void ButelLive::on_m_pSldLeftCtrsliderReleased()
{
    SetAudioPara();
}

void ButelLive::on_m_pSldRightCtrsliderReleased()
{
    SetAudioPara();
}

void ButelLive::on_m_pSldPGMCtrsliderReleased()
{
    SetAudioPara();
}
...
void ButelLive::SetAudioPara()
{
    ...
    soundInfo.bPlayLocalLive = m_bPlayLive;
...
void ButelLive::keyPressEvent(QKeyEvent *event)
{
    ...
    {
        Log::writeMessage(LOG_RTSPSERV, 1, "%s Key_Alt pressed", __FUNCTION__);
//        if(!ui->m_pDisArea->m_bPressed)
        ui->m_pDisArea->SetSourceCropData();
...
bool ButelLive::eventFilter(QObject *obj, QEvent *event)
{
    if(obj == ui->m_pSldLeftCtr || obj == ui->m_pSldRightCtr || obj == ui->m_pSldPGMCtr)
    {
    ...
    else if(obj == ui->m_pDisArea)
    {
        if(event->type() == QEvent::KeyPress)
        {
            ...
            if(!shortCut.isEmpty())
            {
                for(int i = 0; i < this->actions().count(); i++)
                {
                    if(this->actions().at(i)->shortcut().toString() == shortCut)
                    {
                        emit this->actions().at(i)->triggered(true);
                    }
                }
                return true;
            }
...
//        if(ON_BUTTON_OK == ret)
        ...
        if(0 == ret)
        {
            ...
            if((m_LiveSettingParam.liveSettingParam.DelayTime > 0) || (m_LiveSettingParam.liveSettingParam.bUseLiveSec && m_LiveSettingParam.liveSettingParam.DelayTimeSec > 0) && !m_bIsShiftF2)
            {
                m_bIsDelayLive = true;
            }
            else if((m_LiveSettingParam.liveSettingParam.DelayTime > 0 && !m_LiveSettingParam.liveSettingParam.bUseLiveSec) && !m_bIsShiftF2)
            {
                m_bIsDelayLive = true;
            }
            else
...
//直播计时
void ButelLive::OnLiveTimeOut()
{
    m_LiveTime++;
    int sec = m_LiveTime%60%60;
    int hour = m_LiveTime/60/60;
    int mini = (m_LiveTime - 60*60*hour)/60;
    QString strTime = "%1:%2:%3";
    strTime = strTime.arg(hour,2,10,QChar('0')).arg(mini,2,10,QChar('0')).arg(sec,2,10,QChar('0'));
    if(m_bIsLiving)
        ui->m_pLbLiveTime->setText(strTime);
    else if(m_bIsRecording)
        ui->m_pLbRecordTime->setText(strTime);
}
//SDI设置
void ButelLive::on_m_pBtnSDISet_clicked()
{
    SDIConfigureUI* pSDISetUi = new SDIConfigureUI(this);
    pSDISetUi->show();
}
//SDI开关
void ButelLive::on_m_pBtnSDISwitch_clicked()
{
    if(SLiveBlackMagicOutputOn(!m_bSDI) < 0)
    {
        return;
    }
    else
    {
        ...
        ConfigOper::instance()->m_bSDIOutput = m_bSDI;
        ConfigOper::instance()->WriteCfgFile();
    }
}

void ButelLive::on_m_pBtnRecord_clicked()
{
    ...
    if(m_bIsRecording)
    {
        ...
        {
            ui->m_pBtnRecord->setText(tr("StartRecord"));
            m_bIsRecording = false;
            if(!m_bChannelClosed)
            {
                m_pMenuLive->actions().at(1)->setEnabled(true);
            ...
            m_LiveTimer.stop();
            ui->m_pLbRecordTime->setText("00:00:00");
            ui->m_pLbRecordTime->hide();
            UpdateRecordState();
//            CNetInerface::GetInstance()->PauseLive(CNetInerface::GetInstance()->m_StrToken,m_pChannelInfo->id.toInt());
//            int ret = BLiveAppAfterStopPush(OperPause);
        }
    }
    else
    {
        m_bIsNeedRecord = true;
//        CNetInerface::GetInstance()->StartLive(CNetInerface::GetInstance()->m_StrToken,m_pChannelInfo->id.toInt());
        StartLive();
    }
}

void ButelLive::UpdateRecordState()
{
    bool bRecord = false;
    if(SLiveHasIntancesCanRecord(&bRecord) < 0)
    {
...
void ButelLive::SetStartLiveEnable(bool bEnable)
{
    if(bEnable)
    {
        m_pMenuLive->actions().at(1)->setEnabled(true);
...
void ButelLive::OnSaveSceneTimeOut()
{
    Log::writeMessage(LOG_RTSPSERV, 1, "%s ", __FUNCTION__);
    char* ConfigList;
    if(0 == SLiveGetPreConfig(&ConfigList))
...
void ButelLive::on_m_pBtnChat_clicked()
{
    ui->m_pStckWdgt->setCurrentIndex(3);
    ui->m_pBtnChat->setStyleSheet("QPushButton{border-image:url(:images/btn_scene_materal_press.png);color: rgba(255, 255, 255, 255);}");
    ui->m_pBtnMaterial->setStyleSheet("QPushButton{border-image:url(:images/btn_scene_materal.png);color: rgba(255, 255, 255, 255);}");
    ui->m_pBtnScene->setStyleSheet("QPushButton{border-image:url(:images/btn_scene_materal.png);color: rgba(255, 255, 255, 255);}");
}

void ButelLive::on_m_pBtnListenPGM_clicked()
{
    Log::writeMessage(LOG_RTSPSERV, 1, "%s ", __FUNCTION__);
    m_bPlayLive = !m_bPlayLive;
    SetAudioPara();
    if(m_bPlayLive)
    {
        ui->m_pBtnListenPGM->setStyleSheet("QPushButton"
